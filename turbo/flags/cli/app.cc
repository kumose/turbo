...
namespace turbo::cli {
    ...
    App *App::enable_all_flags_option() {
        auto flags_group = add_option_group("all_flags", "system all flags options");
        auto flags = turbo::get_all_flags();
        for (auto &it: flags) {
            std::string flag_name(it.first);
            flags_group->add_option(turbo::str_cat("--", it.first), [](const std::vector<std::string> &r)-> bool {
                TURBO_UNUSED(r);
                return true;
            }, it.second->help())->flags();
        }
        return this;
    }

    App *App::enable_flags_option(std::string_view flag_name) {
        if (const auto cmd = turbo::find_command_line_flag(flag_name); cmd != nullptr) {
            add_option(turbo::str_cat("--", flag_name), [](const std::vector<std::string> &r)-> bool {
                TURBO_UNUSED(r);
                return true;
            }, cmd->help())->flags();
        }
