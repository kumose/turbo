...
#include <turbo/flags/cli/config.h>
#include <algorithm>
#include <string>
#include <utility>
#include <vector>
    ...
    namespace detail {
        ...
        generate_parents(const std::string &section, std::string &name, char parentSeparator) {
            ...
            try {
                ...
                throw turbo::cli::ParseError(iarg.what(), turbo::cli::ExitCodes::InvalidError);
            }
            return parents;
        }
...
/// @brief  checks if a string represents a multiline comment
    ...
    std::vector<ConfigItem> ConfigBase::from_config(std::istream &input) const {
        ...
        while (getline(input, buffer)) {
            ...
            try {
                ...
                throw turbo::cli::ParseError(ia.what(), turbo::cli::ExitCodes::InvalidError);
            }
    ...
    std::string &clean_name_string(std::string &name, const std::string &keyChars) {
                ...
                if (opt->get_configurable()) {
                    ...
                    if (!value.empty()) {
                        ...
                        if (!opt->get_fnames().empty()) {
                            ...
                            } catch (const turbo::cli::ArgumentMismatch &) {
                                bool valid{false};
