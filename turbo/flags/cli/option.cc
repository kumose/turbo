...
namespace turbo::cli {
        ...
        if (!validators_.empty()) {
            if (type_size_max_ > 1) {  // in this context index refers to the index in the type
                ...
                if (get_items_expected_max() < static_cast<int>(res.size()) &&
                    (multi_option_policy_ == turbo::cli::MultiOptionPolicy::TakeLast ||
                     multi_option_policy_ == turbo::cli::MultiOptionPolicy::Reverse)) {
    ...
    int Option::_add_result(std::string &&result, std::vector<std::string> &res) const {
        ...
        if (allow_extra_args_ && !result.empty() && result.front() == '[' &&
            ...
            for (auto &var: turbo::cli::detail::split(result.substr(1), ',')) {
                if (!var.empty()) {
        ...
        if (delimiter_ == '\0') {
            ...
            if ((result.find_first_of(delimiter_) != std::string::npos)) {
                for (const auto &var: turbo::cli::detail::split(result, delimiter_)) {
                    if (!var.empty()) {
                        res.push_back(var);
                        ++result_count;
                    }
