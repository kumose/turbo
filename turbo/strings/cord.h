...
namespace turbo {
    ...
    public:
        ...
        size_t estimated_memory_usage(CordMemoryAccounting accounting_method =
        ...
        public:
            ...
            using pointer = turbo::Nonnull<const value_type *>;
        ...
        private:
            ...
            explicit ChunkIterator(turbo::Nonnull<cord_internal::CordRep *> tree);

            // Constructs a `begin()` iterator from `cord`.
            explicit ChunkIterator(turbo::Nonnull<const Cord *> cord);

            // Initializes this instance from a tree. Invoked by constructors.
            void InitTree(turbo::Nonnull<cord_internal::CordRep *> tree);
            ...
            turbo::Nullable<turbo::cord_internal::CordRep *> current_leaf_ = nullptr;
        ...
        //   }
        ChunkIterator chunk_begin() const TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        ChunkIterator chunk_end() const TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        public:
            ...
            explicit ChunkRange(turbo::Nonnull<const Cord *> cord) : cord_(cord) {}
        ...
        private:
            turbo::Nonnull<const Cord *> cord_;
        };
        ...
        //     }
        //   }
        ChunkRange chunks() const TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        public:
            ...
            using difference_type = ptrdiff_t;
            using pointer = turbo::Nonnull<const char *>;
        ...
        private:
            explicit CharIterator(turbo::Nonnull<const Cord *> cord)
        ...
        static Cord advance_and_read(turbo::Nonnull<CharIterator *> it, size_t n_bytes);
        ...
        static void advance(turbo::Nonnull<CharIterator *> it, size_t n_bytes);
        ...
        CharIterator char_begin() const TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        CharIterator char_end() const TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        public:
            ...
            explicit CharRange(turbo::Nonnull<const Cord *> cord) : cord_(cord) {}
        ...
        //     }
        //   }
        CharRange chars() const TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        std::string_view flatten() TURBO_ATTRIBUTE_LIFETIME_BOUND;
        ...
        // Support automatic stringification with turbo::str_cat and turbo::str_format.
    ...
    private:
        ...
        friend turbo::Nullable<const CordzInfo *> GetCordzInfoForTesting(
                const Cord &cord);
        ...
        public:
            ...
            explicit constexpr InlineRep(std::string_view sv,
                                         turbo::Nullable<CordRep *> rep);

            void Swap(turbo::Nonnull<InlineRep *> rhs);

            size_t size() const;

            // Returns nullptr if holding pointer
            turbo::Nullable<const char *> data() const;

            // Discards pointer, if any
            void set_data(turbo::Nonnull<const char *> data, size_t n);

            turbo::Nonnull<char *> set_data(size_t n);  // Write data to the result
            // Returns nullptr if holding bytes
            turbo::Nullable<turbo::cord_internal::CordRep *> tree() const;

            turbo::Nonnull<turbo::cord_internal::CordRep *> as_tree() const;

            turbo::Nonnull<const char *> as_chars() const;

            // Returns non-null iff was holding a pointer
            turbo::Nullable<turbo::cord_internal::CordRep *> clear();
            ...
            turbo::Nonnull<CordRepFlat *> MakeFlatWithExtraCapacity(size_t extra);
            ...
            void SetTree(turbo::Nonnull<CordRep *> rep, const CordzUpdateScope &scope);

            // Identical to SetTree(), except that `rep` is allowed to be null, in
            // which case the current instance is reset to an empty value.
            void SetTreeOrEmpty(turbo::Nullable<CordRep *> rep,
            ...
            void EmplaceTree(turbo::Nonnull<CordRep *> rep, MethodIdentifier method);

            // Identical to EmplaceTree, except that it copies the parent stack from
            // the provided `parent` data if the parent is sampled.
            void EmplaceTree(turbo::Nonnull<CordRep *> rep, const InlineData &parent,
                             MethodIdentifier method);

            // Commits the change of a newly created, or updated `rep` root value into
            // this cord. `old_rep` indicates the old (inlined or tree) value of the
            // cord, and determines if the commit invokes SetTree() or EmplaceTree().
            void CommitTree(turbo::Nullable<const CordRep *> old_rep,
                            turbo::Nonnull<CordRep *> rep, const CordzUpdateScope &scope,
                            MethodIdentifier method);

            void AppendTreeToInlined(turbo::Nonnull<CordRep *> tree,
                                     MethodIdentifier method);

            void AppendTreeToTree(turbo::Nonnull<CordRep *> tree,
                                  MethodIdentifier method);

            void AppendTree(turbo::Nonnull<CordRep *> tree, MethodIdentifier method);

            void PrependTreeToInlined(turbo::Nonnull<CordRep *> tree,
                                      MethodIdentifier method);

            void PrependTreeToTree(turbo::Nonnull<CordRep *> tree,
                                   MethodIdentifier method);

            void PrependTree(turbo::Nonnull<CordRep *> tree, MethodIdentifier method);

            bool IsSame(const InlineRep &other) const { return data_ == other.data_; }

            void CopyTo(turbo::Nonnull<std::string *> dst) const {
            ...
            void CopyToArray(turbo::Nonnull<char *> dst) const;
            ...
            turbo::Nullable<turbo::cord_internal::CordzInfo *> cordz_info() const {
                return data_.cordz_info();
            }

            // Sets the profiled CordzInfo.
            void set_cordz_info(turbo::Nonnull<cord_internal::CordzInfo *> cordz_info) {
        ...
        static bool GetFlatAux(turbo::Nonnull<turbo::cord_internal::CordRep *> rep,
        ...
        void CopyToArraySlowPath(turbo::Nonnull<char *> dst) const;
        ...
        turbo::Nonnull<turbo::cord_internal::CordRep *> TakeRep() const &;

        turbo::Nonnull<turbo::cord_internal::CordRep *> TakeRep() &&;

        // Helper for append().
        template<typename C>
        void append_impl(C &&src);
        ...
        turbo::Nullable<const crc_internal::CrcCordState *> MaybeGetCrcCordState()
        const;

        CharIterator FindImpl(CharIterator it, std::string_view needle) const;

        void CopyToArrayImpl(turbo::Nonnull<char *> dst) const;
    };
...
namespace turbo {
    ...
    namespace cord_internal {
        ...
        turbo::Nonnull<CordRep *> NewExternalRep(std::string_view data,
                                                 Releaser &&releaser) {
            assert(!data.empty());
        ...
        // NOLINTNEXTLINE - suppress clang-tidy raw pointer return.
        inline turbo::Nonnull<CordRep *> NewExternalRep(
    ...
    Cord make_cord_from_external(std::string_view data, Releaser &&releaser) {
        ...
        if (TURBO_LIKELY(!data.empty())) {
            cord.contents_.EmplaceTree(::turbo::cord_internal::NewExternalRep(
                                       ...
                                       Cord::MethodIdentifier::kmake_cord_from_external);
        } else {
            using ReleaserType = std::decay_t<Releaser>;
    ...
    inline void Cord::InlineRep::Swap(turbo::Nonnull<Cord::InlineRep *> rhs) {
        if (rhs == this) {
            return;
        }
        using std::swap;
    ...
    inline turbo::Nullable<const char *> Cord::InlineRep::data() const {
        return is_tree() ? nullptr : data_.as_chars();
    }

    inline turbo::Nonnull<const char *> Cord::InlineRep::as_chars() const {
        assert(!data_.is_tree());
        return data_.as_chars();
    }

    inline turbo::Nonnull<turbo::cord_internal::CordRep *> Cord::InlineRep::as_tree()
    const {
        assert(data_.is_tree());
        return data_.as_tree();
    }

    inline turbo::Nullable<turbo::cord_internal::CordRep *> Cord::InlineRep::tree()
    const {
        if (is_tree()) {
            return as_tree();
        } else {
    ...
    inline turbo::Nonnull<cord_internal::CordRepFlat *>
    ...
    inline void Cord::InlineRep::EmplaceTree(turbo::Nonnull<CordRep *> rep,
                                             MethodIdentifier method) {
        assert(rep);
        data_.make_tree(rep);
    ...
    inline void Cord::InlineRep::SetTree(turbo::Nonnull<CordRep *> rep,
                                         const CordzUpdateScope &scope) {
        assert(rep);
        assert(data_.is_tree());
        data_.set_tree(rep);
    ...
    inline void Cord::InlineRep::SetTreeOrEmpty(turbo::Nullable<CordRep *> rep,
                                                const CordzUpdateScope &scope) {
        assert(data_.is_tree());
        if (rep) {
            data_.set_tree(rep);
    ...
    inline void Cord::InlineRep::CommitTree(turbo::Nullable<const CordRep *> old_rep,
                                            turbo::Nonnull<CordRep *> rep,
                                            const CordzUpdateScope &scope,
                                            MethodIdentifier method) {
        if (old_rep) {
            SetTree(rep, scope);
    ...
    inline turbo::Nullable<turbo::cord_internal::CordRep *> Cord::InlineRep::clear() {
        if (is_tree()) {
    ...
    inline void Cord::InlineRep::CopyToArray(turbo::Nonnull<char *> dst) const {
        assert(!is_tree());
    ...
    inline std::string_view Cord::flatten() TURBO_ATTRIBUTE_LIFETIME_BOUND {
        turbo::cord_internal::CordRep *rep = contents_.tree();
        if (rep == nullptr) {
            return std::string_view(contents_.data(), contents_.size());
        } else {
    ...
    inline void Cord::CopyToArrayImpl(turbo::Nonnull<char *> dst) const {
    ...
    inline void Cord::ChunkIterator::InitTree(
            turbo::Nonnull<cord_internal::CordRep *> tree) {
    ...
    inline Cord Cord::advance_and_read(turbo::Nonnull<CharIterator *> it,
                                     size_t n_bytes) {
        assert(it != nullptr);
        return it->chunk_iterator_.AdvanceAndReadBytes(n_bytes);
    }

    inline void Cord::advance(turbo::Nonnull<CharIterator *> it, size_t n_bytes) {
        assert(it != nullptr);
