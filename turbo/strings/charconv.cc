...
namespace turbo {
    namespace {
        ...
        struct FloatTraits<double> {
            ...
            static double MakeNan(turbo::Nonnull<const char *> tagp) {
#if TURBO_HAVE_BUILTIN(__builtin_nan)
...
// members and methods.
        ...
        struct FloatTraits<float> {
            ...
            static float MakeNan(turbo::Nonnull<const char *> tagp) {
#if TURBO_HAVE_BUILTIN(__builtin_nanf)
...
// Returns the number of bits shifted.
        int TruncateToBitWidth(int bit_width, turbo::Nonnull<uint128 *> value) {
...
// number is stored in *value.
        ...
        void EncodeResult(const CalculatedFloat &calculated, bool negative,
                          turbo::Nonnull<turbo::from_chars_result *> result,
                          turbo::Nonnull<FloatType *> value) {
...
// shifted left, as necessary.
        uint64_t ShiftRightAndRound(uint128 value, int shift, bool input_exact,
                                    turbo::Nonnull<bool *> output_exact) {
            if (shift <= 0) {
...
// this function returns false) is both fast and correct.
        ...
        template<typename FloatType>
        from_chars_result FromCharsImpl(turbo::Nonnull<const char *> first,
    ...
    from_chars_result from_chars(turbo::Nonnull<const char *> first,
                                 turbo::Nonnull<const char *> last, double &value,
                                 chars_format fmt) {
        return FromCharsImpl(first, last, value, fmt);
    }

    from_chars_result from_chars(turbo::Nonnull<const char *> first,
                                 turbo::Nonnull<const char *> last, float &value,
                                 chars_format fmt) {
        return FromCharsImpl(first, last, value, fmt);
    }
