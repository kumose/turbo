...
//
// Unlike a `std::mutex`, the Turbo `Mutex` provides the following additional
// features:
...
class TURBO_LOCKABLE TURBO_ATTRIBUTE_WARN_UNUSED Mutex {
 public:
  ...
  // Example usage:
  //   namespace foo {
  //   TURBO_CONST_INIT turbo::Mutex mu(turbo::kConstInit);
  //   }
  ...
  TURBO_MUST_USE_RESULT bool TryLock() TURBO_EXCLUSIVE_TRYLOCK_FUNCTION(true);
  ...
  void ReaderLock() TURBO_SHARED_LOCK_FUNCTION();
  ...
  void ReaderUnlock() TURBO_UNLOCK_FUNCTION();
  ...
  TURBO_MUST_USE_RESULT bool ReaderTryLock() TURBO_SHARED_TRYLOCK_FUNCTION(true);
  ...
  void WriterLock() TURBO_EXCLUSIVE_LOCK_FUNCTION() { this->Lock(); }

  void WriterUnlock() TURBO_UNLOCK_FUNCTION() { this->Unlock(); }

  TURBO_MUST_USE_RESULT bool WriterTryLock()
...
class TURBO_SCOPED_LOCKABLE MutexLock {
 public:
  ...
  explicit MutexLock(Mutex* mu) TURBO_EXCLUSIVE_LOCK_FUNCTION(mu) : mu_(mu) {
    this->mu_->Lock();
  }
...
class TURBO_SCOPED_LOCKABLE ReaderMutexLock {
 public:
  explicit ReaderMutexLock(Mutex* mu) TURBO_SHARED_LOCK_FUNCTION(mu) : mu_(mu) {
    mu->ReaderLock();
  }
  ...
  ~ReaderMutexLock() TURBO_UNLOCK_FUNCTION() { this->mu_->ReaderUnlock(); }
...
class TURBO_SCOPED_LOCKABLE WriterMutexLock {
 public:
  explicit WriterMutexLock(Mutex* mu) TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    mu->WriterLock();
  }
  ...
  ~WriterMutexLock() TURBO_UNLOCK_FUNCTION() { this->mu_->WriterUnlock(); }
...
// the face of a throwing `Condition`. (When Turbo is allowed to depend
...
class Condition {
 public:
  ...
  Condition(bool (*func)(T*),
            typename turbo::internal::type_identity<T>::type* arg);
  ...
  // `object->Method()`.
  //
  // Implementation Note: `turbo::internal::type_identity` is used to allow
  // methods to come from base classes. A simpler signature like
  // `Condition(T*, bool (T::*)())` does not suffice.
  template <typename T>
  Condition(T* object,
            bool (turbo::internal::type_identity<T>::type::*method)());

  // Same as above, for const members
  template <typename T>
  Condition(const T* object,
            bool (turbo::internal::type_identity<T>::type::*method)() const);
  ...
  TURBO_CONST_INIT static const Condition kTrue;
...
class CondVar {
 public:
  ...
  //
  // Requires and ensures that the current thread holds the `Mutex`.
  bool WaitWithTimeout(Mutex* mu, turbo::Duration timeout) {
  ...
  //
  // Requires and ensures that the current thread holds the `Mutex`.
  bool WaitWithDeadline(Mutex* mu, turbo::Time deadline) {
...
class TURBO_SCOPED_LOCKABLE MutexLockMaybe {
 public:
  explicit MutexLockMaybe(Mutex* mu) TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    if (this->mu_ != nullptr) {
      this->mu_->Lock();
    }
...
class TURBO_SCOPED_LOCKABLE ReleasableMutexLock {
 public:
  explicit ReleasableMutexLock(Mutex* mu) TURBO_EXCLUSIVE_LOCK_FUNCTION(mu)
      : mu_(mu) {
    this->mu_->Lock();
  }
...
inline constexpr Mutex::Mutex(turbo::ConstInitType) : mu_(0) {}
    ...
    TURBO_FORCE_INLINE Mutex::~Mutex() { Dtor(); }
#endif

#if defined(NDEBUG) && !defined(TURBO_HAVE_THREAD_SANITIZER)
...
TURBO_FORCE_INLINE void Mutex::Dtor() {}
#endif
...
inline Condition::Condition(
    bool (*func)(T*), typename turbo::internal::type_identity<T>::type* arg)
    // Just delegate to the overload above.
    : Condition(func, arg) {}

template <typename T>
inline Condition::Condition(
    T* object, bool (turbo::internal::type_identity<T>::type::*method)())
