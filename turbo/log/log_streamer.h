...
// contents are logged as if by `KLOG` when the `LogStreamer` is destroyed.
...
class LogStreamer final {
 public:
  ...
  explicit LogStreamer(turbo::LogSeverity severity, std::string_view file,
    ...
    // To match `KLOG`'s defaults:
    stream_->setf(std::ios_base::showbase | std::ios_base::boolalpha);
  }

  // A moved-from `turbo::LogStreamer` does not `KLOG` when destroyed,
  ...
  // LogStreamer::~LogStreamer()
  //
  // Logs this LogStreamer's buffered content as if by KLOG.
  ~LogStreamer() {
...
inline LogStreamer LogDebugFatalStreamer(std::string_view file, int line) {
