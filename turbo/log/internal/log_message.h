...
// less represents a particular log message. KLOG/KCHECK macros create a
// temporary instance of `LogMessage` and then stream values to it.  At the end
// of the KLOG/KCHECK statement, LogMessage instance goes out of scope and
...
namespace turbo {
    ...
    namespace log_internal {
        ...
        public:
            ...
            LogMessage(const char *file, int line,
                       InfoTag) TURBO_ATTRIBUTE_COLD TURBO_NOINLINE;

            LogMessage(const char *file, int line,
                       WarningTag) TURBO_ATTRIBUTE_COLD TURBO_NOINLINE;

            LogMessage(const char *file, int line,
                       ErrorTag) TURBO_ATTRIBUTE_COLD TURBO_NOINLINE;
            ...
            // Uses the specified timestamp instead of one collected in the constructor.
            LogMessage &with_app_info(AppInfo info);
            ...
            LogMessage &to_sink_also(turbo::LogSink *sink);

            // Sends this message to `*sink` and no others.  `sink` must not be null.
            LogMessage &to_sink_only(turbo::LogSink *sink);
            ...
            LogMessage &operator<<(const NoLogPrefix&) { return no_prefix(); }

            LogMessage &operator<<(const NoNewLine&) { return no_newline(); }

            LogMessage &operator<<(const AppInfo&info) { return with_app_info(info); }
        ...
        protected:
            ...
            TURBO_NORETURN static void fail_without_stack_trace();

            // Similar to `fail_without_stack_trace()`, but without `abort()`.  Terminates
            // the process with an error exit code.
            TURBO_NORETURN static void fail_quietly();
        ...
        private:
            ...
            void CopyToEncodedBuffer(std::string_view str) TURBO_NOINLINE;

            template<StringType str_type>
            void CopyToEncodedBuffer(char ch, size_t num) TURBO_NOINLINE;
        ...
        // Note: the following is declared `TURBO_NOINLINE`
        template<typename T,
        ...
        // other TUs.  Since the template is marked `TURBO_NOINLINE` we will be
