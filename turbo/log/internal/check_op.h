...
#define TURBO_LOG_INTERNAL_CHECK_OP(name, op, val1, val1_text, val2, val2_text) \
  ...
  TURBO_LOG_INTERNAL_CHECK(*turbo_log_internal_check_op_result).internal_stream()
#define TURBO_LOG_INTERNAL_QCHECK_OP(name, op, val1, val1_text, val2, \
  ...
  TURBO_LOG_INTERNAL_QCHECK(*turbo_log_internal_qcheck_op_result).internal_stream()
...
//   evaluate `.ok()` and (sometimes) `.to_string()`.
...
namespace turbo {
    ...
    namespace log_internal {
        ...
        TURBO_ATTRIBUTE_RETURNS_NONNULL std::string *MakeCheckOpString(
                T1 v1, T2 v2, const char *exprtext) TURBO_NOINLINE;
