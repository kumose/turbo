...
#include <string_view>
#include <vector>
#include <turbo/uri/uriparser/Uri.h>
...
namespace turbo {
  ...
  if (impl_->is_absolute_path_ && impl_->is_file_uri_ && segments.size() > 0 &&
    ...
    turbo::Result<std::vector<std::pair<std::string, std::string>>>
    Uri::query_items() const {
        ...
        if (uriDissectQueryMallocA(&query_list, &item_count, query.first, query.afterLast) != URI_SUCCESS) {
            return turbo::invalid_argument_error("Cannot parse query string: '" + query_string() + "'");
        }
    ...
    turbo::Status Uri::parse(const std::string &uri_string) {
        impl_->Reset();
        ...
        if (uriParseSingleUriExA(&impl_->uri_, s.data(), s.data() + s.size(), &error_pos) !=
            URI_SUCCESS) {
            return turbo::invalid_argument_error("Cannot parse URI: '" + uri_string + "'");
        }

        const auto scheme = TextRangeToView(impl_->uri_.scheme);
        if (scheme.empty()) {
            return turbo::invalid_argument_error("URI has empty scheme: '" + uri_string + "'");
        }
  ...
  if (impl_->is_file_uri_ && impl_->path_segments_.size() > 0) {
    ...
    if (IsDriveSpec(first_seg) && (first_seg.length() >= 3 && first_seg[2] != '/')) {
      impl_->is_absolute_path_ = false;
        ...
        if (impl_->is_file_uri_ && !impl_->is_absolute_path_) {
            return turbo::invalid_argument_error("File URI path must be absolute: '" + uri_string + "'");
        }
        ...
        if (!port_text.empty()) {
            ...
            if (!turbo::simple_atoi(port_text, &port_num)) {
                return turbo::invalid_argument_error("Invalid port number '" + std::string(port_text) + "' in URI '" + uri_string + "'");
            }
            if(port_num > std::numeric_limits<int16_t>::max()) {
                return turbo::invalid_argument_error( "Port number '" + std::string(port_text) + "' in URI '" + uri_string + "' is too large");
            }
    ...
    turbo::Result<Uri> Uri::from_string(const std::string &uri_string) {
        Uri uri;
        auto rs = uri.parse(uri_string);
    ...
    turbo::Result<std::string> uri_from_absolute_path(std::string_view path) {
        if (path.empty()) {
            return turbo::invalid_argument_error("uri_from_absolute_path expected an absolute path, got an empty string");
        }
        std::string uri;
#ifdef _WIN32
