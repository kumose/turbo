...
#include <turbo/memory/launder.h>
...
namespace turbo {
    ...
    public:
        ...
        constexpr T *get() noexcept {
            return turbo::launder(reinterpret_cast<T *>(&data_));
        }

        constexpr const T *get() const noexcept {
            // Use fully qualified name to avoid ambiguities with MSVC (ARROW-14800)
            return turbo::launder(reinterpret_cast<const T *>(&data_));
        }
        ...
        static typename std::enable_if<CanMemcpy>::type move_construct_several(
                AlignedStorage *TURBO_RESTRICT src, AlignedStorage *TURBO_RESTRICT dest, size_t n,
        ...
        move_construct_several_and_destroy_source(AlignedStorage *TURBO_RESTRICT src,
                                                  AlignedStorage *TURBO_RESTRICT dest, size_t n,
                                                  size_t memcpy_length) noexcept {
            TURBO_UNUSED(n);
            memcpy(dest->get(), src->get(), memcpy_length * sizeof(T));
        }
        ...
        static void move_construct_several(AlignedStorage *TURBO_RESTRICT src,
                                           AlignedStorage *TURBO_RESTRICT dest,
                                           size_t n) noexcept {
            move_construct_several(src, dest, n, n);
        }

        static void move_construct_several_and_destroy_source(
                AlignedStorage *TURBO_RESTRICT src, AlignedStorage *TURBO_RESTRICT dest,
                size_t n) noexcept {
