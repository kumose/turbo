...
#ifdef TURBO_RESOURCE_POOL_NEED_FREE_ITEM_NUM
#define TURBO_RESOURCE_POOL_FREE_ITEM_NUM_ADD1                \
    (_global_nfree.fetch_add(1, std::memory_order_relaxed))
#define TURBO_RESOURCE_POOL_FREE_ITEM_NUM_SUB1                \
...
template <typename T>
class TURBO_CACHELINE_ALIGNED ResourcePool {
public:
    ...
    public:
        ...
        // we don't want.
#define TURBO_RESOURCE_POOL_GET(CTOR_ARGS)                              \
        /* Fetch local free id */                                       \
        if (_cur_free.nfree) {                                          \
            const ResourceId<T> free_id = _cur_free.ids[--_cur_free.nfree]; \
            *id = free_id;                                              \
            TURBO_RESOURCE_POOL_FREE_ITEM_NUM_SUB1;                   \
        ...
        inline T* get(ResourceId<T>* id) {
            TURBO_RESOURCE_POOL_GET();
        }

        template <typename A1>
        inline T* get(ResourceId<T>* id, const A1& a1) {
            TURBO_RESOURCE_POOL_GET((a1));
        }

        template <typename A1, typename A2>
        inline T* get(ResourceId<T>* id, const A1& a1, const A2& a2) {
            TURBO_RESOURCE_POOL_GET((a1, a2));
        }

#undef TURBO_RESOURCE_POOL_GET
        ...
        inline int return_resource(ResourceId<T> id) {
            ...
            if (_cur_free.nfree < ResourcePool::free_chunk_nitem()) {
                ...
                TURBO_RESOURCE_POOL_FREE_ITEM_NUM_ADD1;
                return 0;
            }
...
#ifdef TURBO_CLEAR_RESOURCE_POOL_AFTER_ALL_THREADS_QUIT
...
template <typename T>
TURBO_THREAD_LOCAL typename ResourcePool<T>::LocalPool*
