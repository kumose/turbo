...
#define TURBO_HAVE_PREFETCH 1
    ...
    //
    TURBO_FORCE_INLINE void prefetch_to_local_cache(
            const void *addr) {
        __builtin_prefetch(addr, 0, 3);
    }

    TURBO_FORCE_INLINE void prefetch_to_local_cache_nta(
            const void *addr) {
        __builtin_prefetch(addr, 0, 0);
    }

    TURBO_FORCE_INLINE void prefetch_to_local_cache_for_write(
            const void *addr) {
...
#define TURBO_HAVE_PREFETCH 1
    ...
    void prefetch(const void *x) {
        prefetch_to_local_cache(x);
    }

    template<>
    inline void prefetch<PREFETCH_HINT_NTA>(const void *x) {
        prefetch_to_local_cache_nta(x);
    }
}  // namespace turbo
