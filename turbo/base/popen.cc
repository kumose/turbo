...
TURBO_FLAG(bool, run_command_through_clone, false,
           "(Linux specific) Run command with clone syscall to "
           "avoid the costly page table duplication");

extern "C" {
int TURBO_WEAK kthread_usleep(uint64_t microseconds);
}
#endif
...
namespace turbo {
    ...
    int read_command_output_through_clone(std::ostream &os, const char *cmd) {
        ...
        if (!child_stack_mem) {
            KLOG(ERROR) << "Fail to alloc stack for the child process";
            rc = -1;
            goto END;
        }
        ...
        if (cpid < 0) {
            PKLOG(ERROR) << "Fail to clone child process";
            rc = -1;
            goto END;
        }
        close(pipe_fd[1]);
        ...
        for (;;) {
            ...
            if (nr > 0) {
                ...
                KLOG(ERROR) << "Encountered error while reading for the pipe";
                break;
            }
        }
...
#else
        return turbo::get_flag(FLAGS_run_command_through_clone)
