...
//
// We have to keep kutil::EndPoint ABI compatible because it is used so widely, and the size of kutil::EndPoint is
...
// Note that since ResourcePool has been implemented in kthread, we copy it into this repo and change its namespace to
// kutil::details. Those two headers will not be published.
...
public:
    ...
    static GlobalEndPointSet* instance() {
        return ::turbo::get_leaky_singleton<GlobalEndPointSet>();
    }
...
public:
    ...
    static ExtendedEndPoint* create(std::string_view sp, EndPoint* ep) {
        ...
        } else if (turbo::starts_with(sp, "unix:")) {
    ...
    static ExtendedEndPoint* create(std::string_view sp, int port, EndPoint* ep) {
        ...
        } else if (turbo::starts_with(sp, "unix:")) { // ignore port
    ...
    static ExtendedEndPoint* address(const EndPoint& ep) {
        ...
        KCHECK(eep) << "fail to address ExtendedEndPoint from EndPoint";
        return eep;
    }
...
private:
    ...
    static ExtendedEndPoint* new_extended_endpoint(sa_family_t family) {
        ...
        if (eep) {
            ...
            KCHECK(old_ref == 0) << "new ExtendedEndPoint has reference " << old_ref;
            KCHECK(eep->_u.sa.sa_family == AF_UNSPEC) << "new ExtendedEndPoint has family " << eep->_u.sa.sa_family << " set";
    ...
    void embed_to(EndPoint* ep) const {
        KCHECK(0 == _id.value >> 32) << "ResourceId beyond index";
        ep->reset();
...
public:
    ...
    void dec_ref(void) {
        ...
        KCHECK(old_ref >= 1) << "ExtendedEndPoint has unexpected reference " << old_ref;
        if (old_ref == 1) {
    ...
    void to(EndPointStr* ep_str) const {
        if (_u.sa.sa_family == AF_UNIX) {
            ...
            KCHECK(ret) << "fail to do inet_ntop";
            snprintf(ep_str->_buf, sizeof(ep_str->_buf), "[%s]:%d", buf, ntohs(_u.in6.sin6_port));
        } else {
            KCHECK(0) << "family " << _u.sa.sa_family << " not supported";
        }
    }
