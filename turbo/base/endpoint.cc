...
#include <turbo/base/endpoint.h>                    // ip_t
...
namespace turbo  {
    ...
    EndPoint::EndPoint(ip_t ip2, int port2) : ip(ip2), port(port2) {
        ...
        if (ExtendedEndPoint::is_extended(*this)) {
            KCHECK(0) << "EndPoint construct with value that points to an extended EndPoint";
            ip = IP_ANY;
            port = 0;
        }
    }
    ...
    ip_t my_ip() {
        return turbo::get_leaky_singleton<MyAddressInfo>()->my_ip;
    }

    const char *my_ip_cstr() {
        return turbo::get_leaky_singleton<MyAddressInfo>()->my_ip_str.c_str();
    }

    const char *my_hostname() {
        return turbo::get_leaky_singleton<MyAddressInfo>()->my_hostname;
    }
    ...
    static short epoll_to_poll_events(uint32_t epoll_events) {
        ...
        KCHECK_EQ((uint32_t) poll_events, epoll_events);
        return poll_events;
    }
    ...
    int tcp_connect(const EndPoint &server, int *self_port, int connect_timeout_ms) {
        ...
        if (connect_timeout_ms <= 0) {
            if (kthread_connect != NULL) {
                rc = kthread_connect(sockfd, (struct sockaddr *) &serv_addr, serv_addr_size);
            } else {
                rc = ::connect(sockfd, (struct sockaddr *) &serv_addr, serv_addr_size);
            }
        } else {
            timespec abstime = turbo::Time::to_timespec(turbo::Time::future_time(turbo::Duration::milliseconds(connect_timeout_ms)));
            if (kthread_timed_connect != NULL) {
                rc = kthread_timed_connect(sockfd, (struct sockaddr *) &serv_addr,
        ...
        if (self_port != NULL) {
            ...
            if (get_local_side(sockfd, &pt) == 0) {
                *self_port = pt.port;
                ...
                KCHECK(false) << "Fail to get the local port of sockfd=" << sockfd;
            }
        }
    ...
    int tcp_listen(EndPoint point) {
...
#else
            KLOG(ERROR) << "Missing def of SO_REUSEADDR while -reuse_addr is on";
            return -1;
#endif
        }
        ...
        if (turbo::get_flag(FLAGS_reuse_port)) {
            ...
            if (setsockopt(sockfd, SOL_SOCKET, SO_REUSEPORT,
                           &on, sizeof(on)) != 0) {
                KLOG(WARNING) << "Fail to setsockopt SO_REUSEPORT of sockfd=" << sockfd;
            }
#else
            KLOG(ERROR) << "Missing def of SO_REUSEPORT while -reuse_port is on";
            return -1;
#endif
        }
