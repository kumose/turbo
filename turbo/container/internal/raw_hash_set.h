...
#error TURBO_SWISSTABLE_ENABLE_GENERATIONS cannot be directly set
       ...
       defined(TURBO_HAVE_HWADDRESS_SANITIZER) || \
...
constexpr size_t NumGenerationBytes() { return sizeof(GenerationType); }
    ...
    TURBO_DLL extern const ctrl_t kEmptyGroup[32];
    ...
    TURBO_DLL extern const ctrl_t kSooControl[17];
    ...
    TURBO_FORCE_INLINE bool ShouldInsertBackwards(
            TURBO_ATTRIBUTE_UNUSED size_t capacity, TURBO_ATTRIBUTE_UNUSED size_t hash,
            TURBO_ATTRIBUTE_UNUSED const ctrl_t *ctrl) {
#if defined(NDEBUG)
        return false;
#else
    ...
    template<class Mask>
    TURBO_FORCE_INLINE auto GetInsertionOffset(
            Mask mask, TURBO_ATTRIBUTE_UNUSED size_t capacity,
            TURBO_ATTRIBUTE_UNUSED size_t hash,
            TURBO_ATTRIBUTE_UNUSED const ctrl_t *ctrl) {
#if defined(NDEBUG)
...
#define TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(x) return x
    ...
    union MaybeInitializedPtr {
        void *get() const { TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(p); }
    ...
    union HeapOrSoo {
        ...
        MaybeInitializedPtr &slot_array() {
            TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN(heap.slot_array);
        }
...
// Converts `n` into the next valid capacity, per `IsValidCapacity`.
    ...
    TURBO_OPTION_HARDENED == 1 || !defined(NDEBUG)
        return true;
#else
        return false;
#endif
    ...
    TURBO_FORCE_INLINE void IterateOverFullSlots(
    ...
    public:
        ...
        // in one TURBO_NOINLINE function in order to minimize binary code
        ...
        TURBO_NOINLINE bool InitializeSlots(CommonFields &c, Alloc alloc,
                                                      ctrl_t soo_slot_h2,
                                                      size_t key_size,
                                                      size_t value_size) {
            assert(c.capacity());
    ...
    TURBO_NOINLINE void DeallocateStandard(CommonFields &common,
    ...
    TURBO_NOINLINE void TransferRelocatable(void *, void *dst, void *src) {
    ...
    private:
        ...
        using RequiresInsertable = typename std::enable_if<
                std::disjunction<std::is_convertible<T, init_type>,
                        is_same_reference<T,reference>>::value,
                int>::type;
    ...
    public:
        ...
        TURBO_NOINLINE explicit raw_hash_set(
        ...
        raw_hash_set(const raw_hash_set &that, const allocator_type &a)
            ...
            IterateOverFullSlots(
                    ...
                    [&](const ctrl_t *that_ctrl,
                        slot_type *that_slot) TURBO_LAMBDA_FORCE_INLINE {
                        if (shift == 0) {
        ...
        TURBO_NOINLINE raw_hash_set(raw_hash_set &&that) noexcept(
        ...
        iterator end() TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return iterator(common().generation_ptr());
        }

        const_iterator begin() const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return const_cast<raw_hash_set *>(this)->begin();
        }

        const_iterator end() const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return iterator(common().generation_ptr());
        }

        const_iterator cbegin() const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return begin();
        }

        const_iterator cend() const TURBO_ATTRIBUTE_LIFETIME_BOUND { return end(); }
        ...
        size_t capacity() const {
            ...
            TURBO_ATTRIBUTE_UNUSED static constexpr bool kEnabled = SooEnabled();
            TURBO_ATTRIBUTE_UNUSED static constexpr size_t kCapacity = SooCapacity();
            TURBO_ASSUME(!kEnabled || cap >= kCapacity);
            return cap;
        }
        ...
        template<class T, RequiresInsertable<T> = 0, class T2 = T,
                ...
                T * = nullptr>
        std::pair<iterator, bool> insert(T &&value) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        template<class T, RequiresInsertable<T> = 0, class T2 = T,
                typename std::enable_if<IsDecomposable<T2>::value, int>::type = 0,
                T * = nullptr>
        iterator insert(const_iterator, T &&value) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        iterator insert(const_iterator,
                        const T &value) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return insert(value).first;
        }

        iterator insert(const_iterator,
                        init_type &&value) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        insert_return_type insert(node_type &&node) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        iterator insert(const_iterator,
                        node_type &&node) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        iterator lazy_emplace(const key_arg<K> &key,
                              F &&f) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        iterator erase(const_iterator first,
                       const_iterator last) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        iterator find(const key_arg<K> &key,
                      size_t hash) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            if (is_soo()) return find_soo(key);
            return find_non_soo(key, hash);
        }

        template<class K = key_type>
        iterator find(const key_arg<K> &key) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            if (is_soo()) return find_soo(key);
            prefetch_heap_block();
        ...
        const_iterator find(const key_arg<K> &key,
                            size_t hash) const TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        std::pair<const_iterator, const_iterator> equal_range(
                const key_arg<K> &key) const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            auto it = find(key);
            if (it != end()) return {it, std::next(it)};
            return {it, it};
        }
        ...
        friend typename std::enable_if<H::template is_hashable<value_type>::value,
                H>::type
        turbo_hash_value(H h, const raw_hash_set &s) {
    ...
    private:
        ...
        inline void destroy_slots() {
            ...
            IterateOverFullSlots(
                    ...
                    [&](const ctrl_t *, slot_type *slot)
                            TURBO_LAMBDA_FORCE_INLINE { this->destroy(slot); });
        }
        ...
        inline void destructor_impl() {
            ...
            if (is_soo()) {
                if (!empty()) {
                    TURBO_SWISSTABLE_IGNORE_UNINITIALIZED(destroy(soo_slot()));
                }
                return;
            }
            destroy_slots();
        ...
        size_t hash_of_internal(slot_type *slot) const {
        ...
        TURBO_NOINLINE static void resize_impl(
            ...
            const ctrl_t soo_slot_h2 =
                    had_soo_slot ? static_cast<ctrl_t>(H2(set->hash_of_internal(set->soo_slot())))
                                 : ctrl_t::kEmpty;
    ...
    protected:
        ...
        iterator iterator_at(size_t i) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return {control() + i, slot_array() + i, common().generation_ptr()};
        }

        const_iterator iterator_at(size_t i) const TURBO_ATTRIBUTE_LIFETIME_BOUND {
...
#undef TURBO_SWISSTABLE_IGNORE_UNINITIALIZED
#undef TURBO_SWISSTABLE_IGNORE_UNINITIALIZED_RETURN
