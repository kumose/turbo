...
void ConstructElements(turbo::internal::type_identity_t<A>& allocator,
...
class Storage {
 public:
  ...
  using SwapInlinedElementsPolicy = std::conditional_t<
      ...
      std::conjunction<turbo::is_trivially_relocatable<ValueType<A>>,
                        std::is_same<A, std::allocator<ValueType<A>>>>::value,
      MemcpyPolicy,
...
#pragma GCC diagnostic pop
  ...
  // TURBO_ATTRIBUTE_NO_SANITIZE_CFI is used because the memory pointed to may be
  ...
  // as a bad cast.
  TURBO_ATTRIBUTE_NO_SANITIZE_CFI Pointer<A> GetInlinedData() {
    return reinterpret_cast<Pointer<A>>(data_.inlined.inlined_data);
  }

  TURBO_ATTRIBUTE_NO_SANITIZE_CFI ConstPointer<A> GetInlinedData() const {
  ...
  TURBO_NOINLINE void InitFrom(const Storage& other);
 ...
 private:
  TURBO_NOINLINE void DestroyContents();
  ...
  template <typename... Args>
  TURBO_NOINLINE Reference<A> EmplaceBackSlow(Args&&... args);
