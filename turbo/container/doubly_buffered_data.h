...
//
// But, when `AllowKthreadSuspended=false', it is not allowed to suspend kthread
// while reading. Otherwise, it may cause deadlock.
//
//
// --- `AllowKthreadSuspended=true' ---
// It is allowed to suspend kthread while reading.
// It is not allowed to use non-Void TLS.
// If kthread will not be suspended while reading, it also makes Read() almost
// lock-free by making Modify() *much* slower.
// If kthread will be suspended while reading, there is competition among
// kthreads using the same Wrapper.
//
...
// see new foreground, then modify background(foreground before flip) again.
    ...
    template<typename T, typename TLS = Void, bool AllowKthreadSuspended = false>
    ...
    public:
        ...
        private:
            TURBO_DISALLOW_COPY_AND_ASSIGN(ScopedPtr);
...
// other times only reset Wrapper inner structure.
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    class DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSGroup {
    public:
        ...
        inline static DoublyBufferedData::Wrapper *get_or_create_tls_data(WrapperTLSId id) {
            ...
            if (_s_tls_blocks == nullptr) {
                ...
                if (TURBO_UNLIKELY(_s_tls_blocks == nullptr)) {
                    KLOG(FATAL) << "Fail to create vector, " << errno;
                    return nullptr;
                }
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    std::mutex DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSGroup::_s_mutex;

    template<typename T, typename TLS, bool AllowKthreadSuspended>
    std::deque<typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSId> *
            DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSGroup::_s_free_ids = nullptr;

    template<typename T, typename TLS, bool AllowKthreadSuspended>
    typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSId
            DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSGroup::_s_id = 0;

    template<typename T, typename TLS, bool AllowKthreadSuspended>
    __thread std::vector<typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSGroup::ThreadBlock *> *
            DoublyBufferedData<T, TLS, AllowKthreadSuspended>::WrapperTLSGroup::_s_tls_blocks = nullptr;

    template<typename T, typename TLS, bool AllowKthreadSuspended>
    class TURBO_CACHELINE_ALIGNED DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Wrapper
    ...
    public:
        ...
        // For `AllowKthreadSuspended=true'.
...
// Called when thread initializes thread-local wrapper.
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Wrapper *
    DoublyBufferedData<T, TLS, AllowKthreadSuspended>::AddWrapper(
            typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Wrapper *w) {
        if (nullptr == w) {
            return nullptr;
        }
        if (w->_control == this) {
        ...
        if (w->_control != nullptr) {
            KLOG(FATAL) << "Get wrapper from tls but control != this";
            return nullptr;
        }
        try {
            w->_control = this;
            std::unique_lock lk(_wrappers_mutex);
...
// Called when thread quits.
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    void DoublyBufferedData<T, TLS, AllowKthreadSuspended>::RemoveWrapper(
            typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Wrapper *w) {
        if (nullptr == w) {
            return;
        }
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    DoublyBufferedData<T, TLS, AllowKthreadSuspended>::DoublyBufferedData()
            : _index(0), _wrapper_key(0) {
        static_assert(!(AllowKthreadSuspended && !IsVoid<TLS>::value),
                      "Forbidden to allow kthread suspended with non-Void TLS");
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    int DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Read(
            typename DoublyBufferedData<T, TLS, AllowKthreadSuspended>::ScopedPtr *ptr) {
        ...
        if (TURBO_LIKELY(w != nullptr)) {
            if (AllowKthreadSuspended) {
                ...
                // no need to lock mutex and kthread is allowed to be suspended.
                w->BeginRead();
                int index = -1;
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    template<typename Fn>
    size_t DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Modify(Fn &fn) {
        ...
        std::unique_lock lk(_modify_mutex);
        ...
        KCHECK_EQ(ret2, ret) << "index=" << _index.load(std::memory_order_relaxed);
        return ret2;
    }

    template<typename T, typename TLS, bool AllowKthreadSuspended>
    template<typename Fn, typename Arg1>
    size_t DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Modify(Fn &fn, const Arg1 &arg1) {
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    template<typename Fn, typename Arg1, typename Arg2>
    size_t DoublyBufferedData<T, TLS, AllowKthreadSuspended>::Modify(
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    template<typename Fn>
    size_t DoublyBufferedData<T, TLS, AllowKthreadSuspended>::ModifyWithForeground(Fn &fn) {
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    template<typename Fn, typename Arg1>
    size_t DoublyBufferedData<T, TLS, AllowKthreadSuspended>::ModifyWithForeground(Fn &fn, const Arg1 &arg1) {
    ...
    template<typename T, typename TLS, bool AllowKthreadSuspended>
    template<typename Fn, typename Arg1, typename Arg2>
    size_t DoublyBufferedData<T, TLS, AllowKthreadSuspended>::ModifyWithForeground(
            Fn &fn, const Arg1 &arg1, const Arg2 &arg2) {
