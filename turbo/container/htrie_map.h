...
#include <turbo/container/htrie_hash.h>
...
class htrie_map {
 private:
  ...
  using ht = turbo::detail_htrie_hash::htrie_hash<CharT, T, Hash, KeySizeT>;
 ...
 public:
   ...
   * Example:
   *
   *     turbo::htrie_map<char, int> map = {{"/foo", 1}, {"/foo/bar", 1}};
   *
   ...
   * Example:
   *
   *     turbo::htrie_map<char, int> map = {{"/foo", 1}, {"/foo/bar", 2}};
   *     auto print = [](turbo::htrie_map<char>::iterator it) {
   *        std::cout << it.key() << "\n";
   *     };
   *
