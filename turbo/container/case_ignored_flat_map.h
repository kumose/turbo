...
namespace turbo {
    ...
    struct CaseIgnoredEqual {
        // NOTE: No overload for turbo::StringPiece. It needs strncasecmp
    ...
    template<typename T>
    class CaseIgnoredFlatMap : public turbo::FlatMap<
            std::string, T, CaseIgnoredHasher, CaseIgnoredEqual> {
    };

    class CaseIgnoredFlatSet : public turbo::FlatSet<
            std::string, CaseIgnoredHasher, CaseIgnoredEqual> {
    };

    template<typename T>
    class CaseIgnoredMultiFlatMap : public turbo::FlatMap<
