...
namespace turbo {
    ...
    protected:
        ...
        bool isSafeToReferenceAfterResize(const void *Elt, size_t NewSize) {
            // Past the end.
            if (TURBO_LIKELY(!isReferenceToStorage(Elt)))
                return true;
        ...
        static const T *reserveForParamAndGetAddressImpl(U *This, const T &Elt,
            ...
            if (TURBO_LIKELY(NewSize <= This->capacity()))
                return &Elt;

            bool ReferencesStorage = false;
            int64_t Index = -1;
            if (!U::TakesParamByValue) {
                if (TURBO_UNLIKELY(This->isReferenceToStorage(&Elt))) {
...
/// skipping destruction.
    ...
    public:
        ...
        reference emplace_back(ArgTypes &&... Args) {
            if (TURBO_UNLIKELY(this->size() >= this->capacity()))
    ...
    template<typename T, unsigned N>
    class TURBO_GSL_OWNER SmallVector;
    ...
    struct CalculateSmallVectorDefaultInlinedElements {
        ...
        // to be much of an issue, because most turbo development happens on 64-bit
    ...
    class TURBO_GSL_OWNER SmallVector : public SmallVectorImpl<T>,
    ...
    extern template
    class turbo::SmallVectorBase<uint64_t>;
...
namespace std {
    ...
    template<typename T>
    inline void
    swap(turbo::SmallVectorImpl<T> &LHS, turbo::SmallVectorImpl<T> &RHS) {
        LHS.swap(RHS);
    }
    ...
    template<typename T, unsigned N>
    inline void
    swap(turbo::SmallVector<T, N> &LHS, turbo::SmallVector<T, N> &RHS) {
        LHS.swap(RHS);
    }
