...
namespace turbo {
    ...
    public:
        ...
        pointer data() noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        //
        // NOTE: only elements within [`data()`, `data() + size()`) are valid.
        const_pointer data() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return storage_.GetIsAllocated() ? storage_.GetAllocatedData()
                                             : storage_.GetInlinedData();
        }
        ...
        reference operator[](size_type i) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        const_reference operator[](size_type i) const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(i < size());
            return data()[i];
        }
        ...
        reference at(size_type i) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        // NOTE: if `i` is not within the required range of `InlinedVector::at(...)`,
        // in both debug and non-debug builds, `std::out_of_range` will be thrown.
        const_reference at(size_type i) const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            if (TURBO_UNLIKELY(i >= size())) {
                base_internal::ThrowStdOutOfRange(
        ...
        reference front() TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        const_reference front() const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(!empty());
            return data()[0];
        }
        ...
        reference back() TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(!empty());
            return data()[size() - 1];
        }
        ...
        const_reference back() const TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(!empty());
            return data()[size() - 1];
        }
        ...
        iterator begin() noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND { return data(); }

        // Overload of `InlinedVector::begin()` that returns a `const_iterator` to
        // the beginning of the inlined vector.
        const_iterator begin() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return data();
        }
        ...
        iterator end() noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return data() + size();
        }

        // Overload of `InlinedVector::end()` that returns a `const_iterator` to the
        // end of the inlined vector.
        const_iterator end() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return data() + size();
        }
        ...
        const_iterator cbegin() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return begin();
        }
        ...
        const_iterator cend() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return end();
        }
        ...
        reverse_iterator rbegin() noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return reverse_iterator(end());
        }

        // Overload of `InlinedVector::rbegin()` that returns a
        // `const_reverse_iterator` from the end of the inlined vector.
        const_reverse_iterator rbegin() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        reverse_iterator rend() noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return reverse_iterator(begin());
        }

        // Overload of `InlinedVector::rend()` that returns a `const_reverse_iterator`
        // from the beginning of the inlined vector.
        const_reverse_iterator rend() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        // vector.
        const_reverse_iterator crend() const noexcept TURBO_ATTRIBUTE_LIFETIME_BOUND {
            return rend();
        }
        ...
        iterator insert(const_iterator pos,
                        const_reference v) TURBO_ATTRIBUTE_LIFETIME_BOUND {
...
#pragma GCC diagnostic pop
        ...
        iterator insert(const_iterator pos, ForwardIterator first,
                        ForwardIterator last) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(pos >= begin());
            TURBO_HARDENING_ASSERT(pos <= end());
        ...
        iterator insert(const_iterator pos, InputIterator first,
                        InputIterator last) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(pos >= begin());
            TURBO_HARDENING_ASSERT(pos <= end());
...
#pragma GCC diagnostic pop
        ...
        template<typename... Args>
        reference emplace_back(Args &&... args) TURBO_ATTRIBUTE_LIFETIME_BOUND {
        ...
        iterator erase(const_iterator pos) TURBO_ATTRIBUTE_LIFETIME_BOUND {
            TURBO_HARDENING_ASSERT(pos >= begin());
            TURBO_HARDENING_ASSERT(pos < end());
...
#pragma GCC diagnostic pop
        ...
        iterator erase(const_iterator from,
                       const_iterator to) TURBO_ATTRIBUTE_LIFETIME_BOUND {
    ...
    private:
        ...
        friend H turbo_hash_value(H h, const turbo::InlinedVector<TheT, TheN, TheA> &a);
        ...
        void DestroyExistingAndAdopt(InlinedVector &&other) {
            TURBO_HARDENING_ASSERT(other.storage_.GetIsAllocated());
    ...
    H turbo_hash_value(H h, const turbo::InlinedVector<T, N, A> &a) {
        auto size = a.size();
        return H::combine(H::combine_contiguous(std::move(h), a.data(), size), size);
    }

    template<typename T, size_t N, typename A>
    struct sequence_container<turbo::InlinedVector<T, N, A>> : public std::true_type {
        using value_type = T;
    };
