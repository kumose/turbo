...
//
// this from https://github.com/Tessil/hat-trie
...
#include <turbo/container/trie/array_hash.h>
 ...
 */
template <class CharT, class Hash = turbo::ah::str_hash<CharT>,
          class KeyEqual = turbo::ah::str_equal<CharT>,
          bool StoreNullTerminator = true, class KeySizeT = std::uint16_t,
          class IndexSizeT = std::uint32_t,
          class GrowthPolicy = turbo::ah::power_of_two_growth_policy<2>>
class array_set {
 private:
  template <typename U>
  using is_iterator = turbo::detail_array_hash::is_iterator<U>;

  using ht = turbo::detail_array_hash::array_hash<CharT, void, Hash, KeyEqual,
...
/**
 * Same as
 * `turbo::array_set<CharT, Hash, KeyEqual, StoreNullTerminator, KeySizeT,
 * IndexSizeT, turbo::ah::prime_growth_policy>`.
 */
template <class CharT, class Hash = turbo::ah::str_hash<CharT>,
          class KeyEqual = turbo::ah::str_equal<CharT>,
          bool StoreNullTerminator = true, class KeySizeT = std::uint16_t,
