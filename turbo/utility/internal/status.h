...
namespace turbo {
    ...
    enum class StatusCode : int {
        ...
        // StatusCode::kIOError
        //
        // kIOError indicates that the request
        // does not have valid authentication credentials for the operation. Correct
        // the authentication and try again.
        kIOError = 17,
    ...
    // `turbo::Status::to_string()` should construct the output string for a non-ok
    // status.
    enum class StatusToStringMode : int {
        // to_string will not contain any extra data (such as payloads). It will only
        // contain the error code and message, if any.
        kWithNoExtraData = 0,
        // to_string will contain the payloads.
        kWithPayload = 1 << 0,
        // to_string will include all the extra data this Status has.
        kWithEverything = ~kWithNoExtraData,
        // Default mode used by to_string. Its exact value might change in the future.
    ...
    class TURBO_EXPORT StatusPayload {
    public:
        virtual ~StatusPayload() = default;
        ...
        bool operator==(const StatusPayload &other) const noexcept {
            return std::string_view(type_id()) == std::string_view(other.type_id()) && to_string() == other.to_string();
        }
    ...
    struct KVStatusPayload : public StatusPayload {

        explicit KVStatusPayload(std::string_view i, std::string_view v);
    ...
    inline KVStatusPayload::KVStatusPayload(std::string_view i, std::string_view v) : id(i), value(v) {}

    inline const char *KVStatusPayload::type_id() const {
        return id.c_str();
    }

    inline std::string KVStatusPayload::to_string() const {
        return value;
    }

    struct CordStatusPayload : public StatusPayload {

        explicit CordStatusPayload(std::string_view i, turbo::Cord v);
        ...
        std::string id;
        mutable turbo::Cord value;
    };

    inline CordStatusPayload::CordStatusPayload(std::string_view i, turbo::Cord v) : id(i), value(std::move(v)) {}

    inline const char *CordStatusPayload::type_id() const {
        return id.c_str();
    }

    inline std::string CordStatusPayload::to_string() const {
        return std::string(value.flatten());
    }
    ...
    struct TraceStatusPayload : public StatusPayload {

        static const std::string kStackKey;
        ...
        void add_stack(std::string_view msg, TraceParam param);

        uint32_t level{0};
        mutable turbo::Cord stack;
    };

    inline const char *TraceStatusPayload::type_id() const {
        return kStackKey.c_str();
    }
    ...
    public:
        ...
        Status with_message(const Args &...args) const;
        ...
        template<typename T = int, std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>, int> = 0>
        T raw_code() const;

        template<typename T, std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>, int> = 0>
        bool is_raw_code(T v) const;
        ...
        // `operator<<` or `Status::to_string()` for debug logging.
        ...
        template<typename... Args>
        void warn(const turbo::FormatSpec<Args...> &fmt, const Args &... args) const {
            warn(turbo::str_format(fmt, args...));
        }

        // Status::warn()
        //
        // NOTE: This function does nothing if the Status is ok.
        void abort(std::string_view msg = "") const;

        template<typename... Args>
        void abort(const turbo::FormatSpec<Args...> &fmt, const Args &... args) const {
            abort(turbo::str_format(fmt, args...));
        }
        ...
        std::shared_ptr<StatusPayload> get_payload(std::string_view type_url) const;

        std::optional<std::string> get_kv_payload(std::string_view type_url) const;

        std::optional<turbo::Cord> get_cord_payload(std::string_view type_url) const;
        ...
        //
        // NOTE: This function does nothing if the Status is ok.
        Status &set_payload(std::shared_ptr<StatusPayload> payload);

        Status &set_cord_payload(std::string_view type_url, turbo::Cord payload);

        Status &set_kv_payload(std::string_view type_url, std::string_view value);

        /// push call stack to status
        // see TURBO_STATUS_TRACE_PARAM
        Status &push_stack(TraceParam p, std::string_view msg = "");
        ...
        static Status aborted_error(std::string_view message);

        static Status data_loss_error(std::string_view message);

        static Status already_exists_error(std::string_view message);

        static Status cancelled_error(std::string_view message);

        static Status deadline_exceeded_error(std::string_view message);

        static Status failed_precondition_error(std::string_view message);

        static Status internal_error(std::string_view message);

        static Status invalid_argument_error(std::string_view message);

        static Status not_found_error(std::string_view message);

        static Status out_of_range_error(std::string_view message);

        static Status permission_denied_error(std::string_view message);

        static Status resource_exhausted_error(std::string_view message);

        static Status unauthenticated_error(std::string_view message);

        static Status unavailable_error(std::string_view message);

        static Status unimplemented_error(std::string_view message);

        static Status unknown_error(std::string_view message);

        static Status io_error(std::string_view message);

        template<typename T = int, std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>, int> = 0>
        static turbo::Status make_status(T code, std::string_view msg = std::string_view());

        // errno_to_status()
        //
        // Convenience function that creates a `turbo::Status` using an `error_number`,
        // which should be an `errno` value.
        static Status errno_to_status(int error_number, std::string_view message);
    ...
    template<typename T, std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>, int>>
    inline turbo::Status Status::make_status(T code, std::string_view msg) {
        return turbo::Status(static_cast<turbo::StatusCode>(code), msg);
    }
    //------------------------------------------------------------------------------
    ...
    template<typename ...Args>
    Status Status::with_message(const Args &...args) const {
        return Status(raw_code<turbo::StatusCode>(), turbo::str_cat(message(), "; ", args...));
    }
    ...
    template<typename T, std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>, int>>
    inline T Status::raw_code() const {
        static_assert(sizeof(T) >= sizeof(turbo::StatusCode), "type size must less than sizeof(turbo::StatusCode)(4)");
        if (IsInlined(rep_)) return static_cast<T>(InlinedRepToCode(rep_));
        return static_cast<T>(RepToPointer(rep_)->code());
    }

    template<typename T, std::enable_if_t<std::is_integral_v<T> || std::is_enum_v<T>, int>>
    inline bool Status::is_raw_code(T v) const {
        static_assert(sizeof(v) <= sizeof(turbo::StatusCode),
                      "type size must larger than sizeof(turbo::StatusCode)(4)");
        return raw_code<int>() == static_cast<int>(v);
    }

    inline const std::string &Status::message() const {
        return !IsInlined(rep_)
               ? RepToPointer(rep_)->message()
               : (IsMovedFrom(rep_) ? kMovedFromString
                                    : kEmptyString);
    }
    ...
    inline std::shared_ptr<StatusPayload> Status::get_payload(
    ...
    inline std::optional<std::string> Status::get_kv_payload(std::string_view type_url) const {
        if (IsInlined(rep_)) {
            return std::nullopt;
        }
        const auto &ptr = get_payload(type_url);
        if (ptr) {
            return static_cast<const KVStatusPayload *>(ptr.get())->value;
        }
        return std::nullopt;
    }

    inline std::optional<turbo::Cord> Status::get_cord_payload(std::string_view type_url) const {
        if (IsInlined(rep_)) {
            return std::nullopt;
        }
        const auto &ptr = get_payload(type_url);
        if (ptr) {
            return static_cast<const CordStatusPayload *>(ptr.get())->value;
        }
        return std::nullopt;
    }

    inline std::string Status::get_stack() const {
        if (IsInlined(rep_)) {
            return "";
        }
        const auto &ptr = get_payload(TraceStatusPayload::kStackKey);
        if (ptr) {
    ...
    inline Status &Status::set_payload(std::shared_ptr<StatusPayload> payload) {
        if (ok()) return *this;
        status_internal::StatusRep *rep = PrepareToModify(rep_);
        rep->set_payload(std::move(payload));
        rep_ = PointerToRep(rep);
        return *this;
    }

    inline Status &Status::set_kv_payload(std::string_view type_url, std::string_view value) {
        if (ok()) return *this;
        auto pay = std::make_shared<KVStatusPayload>(type_url, value);
        return set_payload(pay);
    }

    inline Status &Status::set_cord_payload(std::string_view type_url, turbo::Cord payload) {
        if (ok()) return *this;
        auto pay = std::make_shared<CordStatusPayload>(type_url, payload);
        return set_payload(pay);
    }

    inline Status &Status::push_stack(TraceParam p, std::string_view msg) {
        if (ok()) {
            return *this;
        }
        auto ppay = get_payload(TraceStatusPayload::kStackKey);
        if (!ppay) {
            auto ptr = std::make_shared<TraceStatusPayload>();
            set_payload(ptr);
            ppay = get_payload(TraceStatusPayload::kStackKey);
        }
        static_cast<TraceStatusPayload *>(ppay.get())->add_stack(msg, p);
        return *this;
    }
...
#define TURBO_STATUS_TRACE_PARAM ::turbo::TraceParam{__FILE__, __LINE__, TURBO_PRETTY_FUNCTION, ::turbo::TraceStatusPayload::kMaxPathDepth}