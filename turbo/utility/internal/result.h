...
namespace turbo {
    ...
    //   DoWork(v.value_or_die());  // Accessing value_or_die() when not "OK" may throw
...
// Returned Result objects may not be ignored.
    ...
    //   }
    //
    // NOTE: using `turbo::Result<T>::value_or_die()` when no valid value is present will
    ...
    //
    //   Result<Foo> result = DoBigCalculationThatCouldFail();
    //   const Foo& foo = result.value_or_die();    // Crash/exception if no value present
    //   foo.DoSomethingCool();
    //
    // A `turbo::Result<T*>` can be constructed from a null pointer like any other
    // pointer value, and the result will be that `ok()` returns `true` and
    // `value_or_die()` returns `nullptr`. Checking the value of pointer in an
    ...
    public:
        ...
        // this constructor, `this->ok()` will be `false` and calls to `value_or_die()` will
        ...
        // Status::warn()
        //
        // no effect if ok() = true;
        void warn(std::string_view msg = "") const {
            this->status_.warn(msg);
        }
        ...
        //
        //   T value = statusor.value_or_die();
        //
        // Otherwise, if the value type is expensive to copy, but can be left
        // in the Result, simply assign to a reference:
        //
        //   T& value = statusor.value_or_die();  // or `const T&`
        //
        // Otherwise, if the value type supports an efficient move, it can be
        // used as follows:
        //
        //   T value = std::move(statusor).value_or_die();
        //
        ...
        const T &value_or_die() const & TURBO_ATTRIBUTE_LIFETIME_BOUND;

        T &value_or_die() & TURBO_ATTRIBUTE_LIFETIME_BOUND;

        T value_or_die() && TURBO_ATTRIBUTE_LIFETIME_BOUND;

        template<typename U, std::enable_if_t<std::is_convertible_v<T, U>, int> = 0>
        turbo::Status try_value(turbo::Nonnull<U *> out) &&;

        constexpr const T &value_unsafe() const &;

        constexpr T &value_unsafe() &;
        ...
        // `turbo::Result<T>`. Alternatively, see the `value_or_die()` member function for a
        ...
        /// the stored error.
        template<typename M>
        typename EnsureResult<decltype(std::declval<M &&>()(std::declval<T &&>()))>::type
        mapping(
                M &&m) &&{
            if (!ok()) {
                return status();
    ...
    void turbo_stringify(Sink &sink, const Result<T> &status_or) {
        if (status_or.ok()) {
            turbo::format(&sink, "%v", status_or.value_or_die());
        } else {
    ...
    template<typename T>
    const T &Result<T>::value_or_die() const &{
    ...
    template<typename T>
    template<typename U, std::enable_if_t<std::is_convertible_v<T, U>, int>>
    turbo::Status Result<T>::try_value(turbo::Nonnull<U *> out) &&{
        if (this->ok()) {
    ...
    template<typename T>
    constexpr const T &Result<T>::value_unsafe() const &{ return *this->storage_.get(); }

    template<typename T>
    constexpr T &Result<T>::value_unsafe() &{ return *this->storage_.get(); }

    template<typename T>
    T Result<T>::value_unsafe() &&{ return move_value_unsafe(); }

    template<typename T>
    T Result<T>::move_value_unsafe() {
    ...
    struct EmptyResult {
        static Result<EmptyResult> to_result(Status s) {
            if (TURBO_LIKELY(s.ok())) {
                return EmptyResult{};
            }
            return s;
        }
