        ...
        inline Duration MakeDurationFromU128(uint128 u128, bool is_neg) {
            ...
            if (h64 == 0) {  // fastpath
                ...
                if (h64 >= kMaxRepHi64) {
                    ...
                    return is_neg ? Duration::min_infinite() : Duration::max_infinite();
                }
...
// double as overflow cases.
        inline bool SafeAddRepHi(double a_hi, double b_hi, Duration *d) {
            ...
            if (c >= static_cast<double>(kint64max)) {
                *d = Duration::max_infinite();
                return false;
            }
            if (c <= static_cast<double>(kint64min)) {
                *d = Duration::min_infinite();
                return false;
            }
...
// T value instead of overflowing. This is only defined for uint128.
        ...
        struct SafeMultiply {
            uint128 operator()(uint128 a, uint128 b) const {
                ...
                return b == 0 ? b : (a > uint128_max() / b) ? uint128_max() : a * b;
            }
        };
...
// in *rem and true will be returned.
    ...
    namespace {
        ...
        int64_t IDivSlowPath(bool satq, const Duration num, const Duration den,
            ...
            if (time_internal::IsInfiniteDuration(num) || den == Duration::zero()) {
                *rem = num_neg ? Duration::min_infinite() : Duration::max_infinite();
        ...
        TURBO_FORCE_INLINE int64_t IDivDurationImpl(bool satq,
                                                                      const Duration num,
                                                                      const Duration den,
                                                                      Duration *rem) {
            int64_t q = 0;
    ...
    int64_t Duration::idiv(Duration num, Duration den, Duration *rem) {
    ...
    Duration &Duration::operator+=(Duration rhs) {
        ...
        if (rhs.rep_hi_.Get() < 0 ? rep_hi_.Get() > orig_rep_hi
            ...
            return *this =
                           rhs.rep_hi_.Get() < 0 ? Duration::min_infinite() : Duration::max_infinite();
        }
        return *this;
    }
    ...
    Duration &Duration::operator-=(Duration rhs) {
        ...
        if (time_internal::IsInfiniteDuration(rhs)) {
            return *this = rhs.rep_hi_.Get() >= 0 ? Duration::min_infinite()
                                                  : Duration::max_infinite();
        }
        const int64_t orig_rep_hi = rep_hi_.Get();
        rep_hi_ = DecodeTwosComp(EncodeTwosComp(rep_hi_.Get()) -
...
//
    ...
    Duration &Duration::operator*=(int64_t r) {
        if (time_internal::IsInfiniteDuration(*this)) {
            ...
            return *this = is_neg ? Duration::min_infinite() : Duration::max_infinite();
        }
    ...
    double Duration::fdiv(Duration num, Duration den) {
    ...
    //
    // Duration::trunc/Duration::floor/Duration::ceil.
    //

    Duration Duration::trunc(Duration d, Duration unit) { return d - (d % unit); }

    Duration Duration::floor(const Duration d, const Duration unit) {
        const turbo::Duration td = Duration::trunc(d, unit);
        return td <= d ? td : td - Duration::abs(unit);
    }

    Duration Duration::ceil(const Duration d, const Duration unit) {
        const turbo::Duration td = Duration::trunc(d, unit);
        return td >= d ? td : td + Duration::abs(unit);
    }
