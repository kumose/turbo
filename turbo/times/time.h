...
namespace turbo {
    ...
    /// yield cpu for sleep
    /// 1 millisecond
    inline void sleep_a_bit() {
        sleep_for(turbo::Duration::milliseconds(1));
    }
...
namespace turbo {
    ...
    class TimeCost {
    public:
        TimeCost():_start(turbo::Time::current_time()){}
        ~TimeCost() = default;

        void reset() {
            _start = turbo::Time::current_time();
        }
        turbo::Time start() const {
            return _start;
        }

        turbo::Duration stop() const {
            return turbo::Time::current_time() - start();
        }

        uint64_t n_elapsed() const {
            return turbo::Duration::to_nanoseconds(stop());
        }

        uint64_t u_elapsed() const {
            return turbo::Duration::to_microseconds(stop());
        }

        uint64_t m_elapsed() const {
            return turbo::Duration::to_milliseconds(stop());
        }
        uint64_t s_elapsed() const {
            return turbo::Duration::to_seconds(stop());
        }
...
namespace std {
    inline std::ostream & operator<<(std::ostream&os, const turbo::TimeCost &tc) {
        os<<tc.stop();
        return os;
    }
}
