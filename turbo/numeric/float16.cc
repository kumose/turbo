...
// Converts a IEEE binary32/64 into a binary16. Rounds to nearest with ties to zero
    ...
    Float16 Float16::from_float(float f) {
        const uint32_t f_bits = bit_cast<uint32_t>(f);
        return from_bits(BinaryConverter<uint32_t>::ToBinary16(f_bits));
    }

    double Float16::to_double() const {
        const uint64_t d_bits = BinaryConverter<uint64_t>::FromBinary16(bits_);
        return bit_cast<double>(d_bits);
    }

    Float16 Float16::from_double(double d) {
        const uint64_t d_bits = bit_cast<uint64_t>(d);
        return from_bits(BinaryConverter<uint64_t>::ToBinary16(d_bits));
    }

    std::ostream &operator<<(std::ostream &os, Float16 arg) { return (os << arg.to_float()); }
