...
#if TURBO_HAVE_BUILTIN(__builtin_clzg)
            return x == 0 ? 16 : __builtin_clzg(x);
#elif TURBO_HAVE_BUILTIN(__builtin_clzs)
    ...
    (defined(_M_X64) || defined(_M_ARM64))
...
#if TURBO_HAVE_BUILTIN(__builtin_ctzg)
            return __builtin_ctzg(x);
#elif TURBO_HAVE_BUILTIN(__builtin_ctzs)
            static_assert(sizeof(unsigned short) == sizeof(x),  // NOLINT(runtime/int)
        ...
        BitCeilNonPowerOf2(T x) {
            ...
            return BitCeilPromotionHelper(static_cast<T>(std::numeric_limits<T>::digits - CountLeadingZeroes(x)),
                                          T{sizeof(T) >= sizeof(unsigned) ? 0 : std::numeric_limits<unsigned>::digits -
                                                                                std::numeric_limits<T>::digits});
        }
