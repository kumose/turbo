...
//
    ...
    public:
        ...
        template<typename H>
        friend H turbo_hash_value(H h, uint128 v) {
...
namespace std {
    ...
    public:
        ...
        TURBO_DISABLE_DEPRECATED_DECLARATION_WARNING
        static constexpr float_denorm_style has_denorm = denorm_absent;
        TURBO_RESTORE_DEPRECATED_DECLARATION_WARNING
...
namespace turbo {
    ...
    public:
        ...
        // Support for turbo::Hash.
        template<typename H>
        friend H turbo_hash_value(H h, int128 v) {
...
#else
      if (uint64_t lhs_high = uint128_high64(lhs), rhs_high = uint128_high64(rhs);
          lhs_high < rhs_high) {
        return turbo::strong_ordering::less;
      } else if (lhs_high > rhs_high) {
        return turbo::strong_ordering::greater;
      } else if (uint64_t lhs_low = uint128_low64(lhs), rhs_low = uint128_low64(rhs);
                 lhs_low < rhs_low) {
        return turbo::strong_ordering::less;
...
#else
        return make_uint128(
            ~uint128_high64(val) + static_cast<unsigned long>(uint128_low64(val) == 0),
            ~uint128_low64(val) + 1);
#endif
    }
...
// Arithmetic operators.
    ...
    constexpr uint128 AddResult(uint128 result, uint128 lhs) {
      // check for carry
      return (uint128_low64(result) < uint128_low64(lhs))
                 ? make_uint128(uint128_high64(result) + 1, uint128_low64(result))
                 : result;
    }
    }  // namespace int128_internal
#endif
    ...
    constexpr uint128 operator+(uint128 lhs, uint128 rhs) {
        ...
        return int128_internal::AddResult(
            make_uint128(uint128_high64(lhs) + uint128_high64(rhs),
                        uint128_low64(lhs) + uint128_low64(rhs)),
            lhs);
#endif
    }
