...
function(carbin_cc_test_library)
    ...
    if ("${CARBIN_CC_TEST_LIB_NAME}" STREQUAL "")
        get_filename_component(CARBIN_CC_TEST_LIB_NAME ${CMAKE_CURRENT_SOURCE_DIR} NAME)
        string(REPLACE " " "_" CARBIN_CC_TEST_LIB_NAME ${CARBIN_CC_TEST_LIB_NAME})
        carbin_print(" Library, NAME argument not provided. Using folder name:  ${CARBIN_CC_TEST_LIB_NAME}")
    endif ()

    if ("${CARBIN_CC_TEST_LIB_NAMESPACE}" STREQUAL "")
        set(CARBIN_CC_TEST_LIB_NAMESPACE ${PROJECT_NAME})
        carbin_print(" Library, NAMESPACE argument not provided. Using target alias:  ${CARBIN_CC_TEST_LIB_NAME}::${CARBIN_CC_TEST_LIB_NAME}")
    endif ()

    carbin_raw("-----------------------------------")
    if (CARBIN_CC_TEST_LIB_SHARED)
        set(CARBIN_LIB_INFO "${CARBIN_CC_TEST_LIB_NAMESPACE}::${CARBIN_CC_TEST_LIB_NAME}  SHARED&STATIC INTERNAL")
    else ()
        set(CARBIN_LIB_INFO "${CARBIN_CC_TEST_LIB_NAMESPACE}::${CARBIN_CC_TEST_LIB_NAME}  STATIC INTERNAL")
    endif ()

    set(${CARBIN_CC_TEST_LIB_NAME}_INCLUDE_SYSTEM SYSTEM)
    if (CARBIN_CC_TEST_LIB_EXCLUDE_SYSTEM)
        set(${CARBIN_CC_TEST_LIB_NAME}_INCLUDE_SYSTEM "")
    endif ()

    carbin_print_label("Create Library" "${CARBIN_LIB_INFO}")
    carbin_raw("-----------------------------------")
    if (VERBOSE_CARBIN_BUILD)
        carbin_print_list_label("Sources" CARBIN_CC_TEST_LIB_SOURCES)
        carbin_print_list_label("Objects" CARBIN_CC_TEST_LIB_OBJECTS)
        carbin_print_list_label("Deps" CARBIN_CC_TEST_LIB_DEPS)
        carbin_print_list_label("COPTS" CARBIN_CC_TEST_LIB_COPTS)
        carbin_print_list_label("CXXOPTS" CARBIN_CC_TEST_LIB_CXXOPTS)
        carbin_print_list_label("CUOPTS" CARBIN_CC_TEST_LIB_CUOPTS)
        carbin_print_list_label("Defines" CARBIN_CC_TEST_LIB_DEFINES)
        carbin_print_list_label("Includes" CARBIN_CC_TEST_LIB_INCLUDES)
        carbin_print_list_label("Private Includes" CARBIN_CC_TEST_LIB_PINCLUDES)
        carbin_print_list_label("Links" CARBIN_CC_TEST_LIB_LINKS)
        carbin_print_list_label("Private Links" CARBIN_CC_TEST_LIB_PLINKS)
        carbin_raw("-----------------------------------")
    endif ()
    set(CARBIN_CC_TEST_LIB_OBJECTS_FLATTEN)
    if (CARBIN_CC_TEST_LIB_OBJECTS)
        foreach (obj IN LISTS CARBIN_CC_TEST_LIB_OBJECTS)
            list(APPEND CARBIN_CC_TEST_LIB_OBJECTS_FLATTEN $<TARGET_OBJECTS:${obj}>)
        endforeach ()
    endif ()
    if (CARBIN_CC_TEST_LIB_SOURCES)
        add_library(${CARBIN_CC_TEST_LIB_NAME}_OBJECT OBJECT ${CARBIN_CC_TEST_LIB_SOURCES} ${CARBIN_CC_TEST_LIB_HEADERS})
        list(APPEND CARBIN_CC_TEST_LIB_OBJECTS_FLATTEN $<TARGET_OBJECTS:${CARBIN_CC_TEST_LIB_NAME}_OBJECT>)
        if (CARBIN_CC_TEST_LIB_DEPS)
            add_dependencies(${CARBIN_CC_TEST_LIB_NAME}_OBJECT ${CARBIN_CC_TEST_LIB_DEPS})
        endif ()
        set_property(TARGET ${CARBIN_CC_TEST_LIB_NAME}_OBJECT PROPERTY POSITION_INDEPENDENT_CODE 1)
        target_compile_options(${CARBIN_CC_TEST_LIB_NAME}_OBJECT PRIVATE $<$<COMPILE_LANGUAGE:C>:${CARBIN_CC_TEST_LIB_COPTS}>)
        target_compile_options(${CARBIN_CC_TEST_LIB_NAME}_OBJECT PRIVATE $<$<COMPILE_LANGUAGE:CXX>:${CARBIN_CC_TEST_LIB_CXXOPTS}>)
        target_compile_options(${CARBIN_CC_TEST_LIB_NAME}_OBJECT PRIVATE $<$<COMPILE_LANGUAGE:CUDA>:${CARBIN_CC_TEST_LIB_CUOPTS}>)
        target_include_directories(${CARBIN_CC_TEST_LIB_NAME}_OBJECT ${${CARBIN_CC_TEST_LIB_NAME}_INCLUDE_SYSTEM}
                PUBLIC
                ${CARBIN_CC_TEST_LIB_INCLUDES}
                "$<BUILD_INTERFACE:${${PROJECT_NAME}_SOURCE_DIR}>"
                "$<BUILD_INTERFACE:${${PROJECT_NAME}_BINARY_DIR}>"
                "$<INSTALL_INTERFACE:${CMAKE_INSTALL_INCLUDEDIR}>"
        )
        target_include_directories(${CARBIN_CC_TEST_LIB_NAME}_OBJECT ${${CARBIN_CC_TEST_LIB_NAME}_INCLUDE_SYSTEM}
                PRIVATE
                ${CARBIN_CC_TEST_LIB_PINCLUDES}
        )

        target_compile_definitions(${CARBIN_CC_TEST_LIB_NAME}_OBJECT
                PUBLIC
                ${CARBIN_CC_TEST_LIB_DEFINES}
        )
    endif ()

    list(LENGTH CARBIN_CC_TEST_LIB_OBJECTS_FLATTEN obj_len)
    if (obj_len EQUAL -1)
        carbin_error("no source or object give to the library ${CARBIN_CC_TEST_LIB_NAME}")
    endif ()
    add_library(${CARBIN_CC_TEST_LIB_NAME}_static STATIC ${CARBIN_CC_TEST_LIB_OBJECTS_FLATTEN})
    if (${CARBIN_CC_TEST_LIB_NAME}_OBJECT)
        add_dependencies(${CARBIN_CC_TEST_LIB_NAME}_static ${CARBIN_CC_TEST_LIB_NAME}_OBJECT)
    endif ()
    if (CARBIN_CC_TEST_LIB_DEPS)
        add_dependencies(${CARBIN_CC_TEST_LIB_NAME}_static ${CARBIN_CC_TEST_LIB_DEPS})
    endif ()
    target_link_libraries(${CARBIN_CC_TEST_LIB_NAME}_static PRIVATE ${CARBIN_CC_TEST_LIB_PLINKS})
    target_link_libraries(${CARBIN_CC_TEST_LIB_NAME}_static PUBLIC ${CARBIN_CC_TEST_LIB_LINKS})
    target_link_libraries(${CARBIN_CC_TEST_LIB_NAME}_static PRIVATE ${CARBIN_CC_TEST_LIB_WLINKS})
    set_target_properties(${CARBIN_CC_TEST_LIB_NAME}_static PROPERTIES
            OUTPUT_NAME ${CARBIN_CC_TEST_LIB_NAME})
    add_library(${CARBIN_CC_TEST_LIB_NAMESPACE}::${CARBIN_CC_TEST_LIB_NAME}_static ALIAS ${CARBIN_CC_TEST_LIB_NAME}_static)
    if (CARBIN_CC_TEST_LIB_SHARED)
        add_library(${CARBIN_CC_TEST_LIB_NAME}_shared SHARED ${CARBIN_CC_TEST_LIB_OBJECTS_FLATTEN})
        if (${CARBIN_CC_TEST_LIB_NAME}_OBJECT)
            add_dependencies(${CARBIN_CC_TEST_LIB_NAME}_shared ${CARBIN_CC_TEST_LIB_NAME}_OBJECT)
        endif ()
        if (CARBIN_CC_TEST_LIB_DEPS)
            add_dependencies(${CARBIN_CC_TEST_LIB_NAME}_shared ${CARBIN_CC_TEST_LIB_DEPS})
        endif ()
        target_link_libraries(${CARBIN_CC_TEST_LIB_NAME}_shared PRIVATE ${CARBIN_CC_TEST_LIB_PLINKS})
        target_link_libraries(${CARBIN_CC_TEST_LIB_NAME}_shared PRIVATE ${CARBIN_CC_TEST_LIB_LINKS})
        foreach (link ${CARBIN_CC_TEST_LIB_WLINKS})
            target_link_libraries(${CARBIN_CC_TEST_LIB_NAME}_shared PRIVATE $<LINK_LIBRARY:WHOLE_ARCHIVE,${link}>)
        endforeach ()
        set_target_properties(${CARBIN_CC_TEST_LIB_NAME}_shared PROPERTIES
                OUTPUT_NAME ${CARBIN_CC_TEST_LIB_NAME}
                VERSION ${${PROJECT_NAME}_VERSION}
                SOVERSION ${${PROJECT_NAME}_VERSION_MAJOR})
        add_library(${CARBIN_CC_TEST_LIB_NAMESPACE}::${CARBIN_CC_TEST_LIB_NAME} ALIAS ${CARBIN_CC_TEST_LIB_NAME}_shared)
    endif ()

    foreach (arg IN LISTS CARBIN_CC_TEST_LIB_UNPARSED_ARGUMENTS)
        message(WARNING "Unparsed argument: ${arg}")
    endforeach ()
...
function(carbin_cc_test)
    ...
    if (CARBIN_CC_TEST_EXCLUDE_SYSTEM)
...
function(carbin_cc_binary)
    ...
    if (CARBIN_CC_BINARY_EXCLUDE_SYSTEM)
    ...
    target_include_directories(${exec_case} ${${CARBIN_CC_BINARY_NAME}_INCLUDE_SYSTEM}
            PRIVATE
...
function(carbin_cc_bm)
    ...
    if (CARBIN_CC_BM_EXCLUDE_SYSTEM)
