...
//
    ...
    void BM_Duration_FDivDuration_Nanoseconds(benchmark::State &state) {
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(
                    d += turbo::Duration::fdiv(turbo::Duration::milliseconds(i), turbo::Duration::nanoseconds(1)));
            ++i;
        }
    }
    ...
    void BM_Duration_IDivDuration_Nanoseconds(benchmark::State &state) {
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(a +=
                                             turbo::Duration::idiv(turbo::Duration::nanoseconds(i),
    ...
    void BM_Duration_IDivDuration_Microseconds(benchmark::State &state) {
        ...
        int i = 0;
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(a += turbo::Duration::idiv(turbo::Duration::microseconds(i),
    ...
    void BM_Duration_IDivDuration_Milliseconds(benchmark::State &state) {
        ...
        int i = 0;
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(a += turbo::Duration::idiv(turbo::Duration::milliseconds(i),
                                                                turbo::Duration::milliseconds(1),
                                                                &ignore));
            ++i;
        }
    ...
    void BM_Duration_IDivDuration_Seconds(benchmark::State &state) {
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(
                    a += turbo::Duration::idiv(turbo::Duration::seconds(i), turbo::Duration::seconds(1), &ignore));
            ++i;
        }
    }
    ...
    void BM_Duration_IDivDuration_Minutes(benchmark::State &state) {
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(
                    a += turbo::Duration::idiv(turbo::Duration::minutes(i), turbo::Duration::minutes(1), &ignore));
            ++i;
        }
    }
    ...
    void BM_Duration_IDivDuration_Hours(benchmark::State &state) {
        ...
        while (state.KeepRunning()) {
            benchmark::DoNotOptimize(
                    a += turbo::Duration::idiv(turbo::Duration::hours(i), turbo::Duration::hours(1), &ignore));
            ++i;
        }
    }
    ...
    void BM_Duration_Modulo(benchmark::State &state) {
        int i = 0;
        while (state.KeepRunning()) {
            auto mod = turbo::Duration::seconds(i) % turbo::Duration::nanoseconds(12345);
    ...
    void BM_Duration_Modulo_FastPath(benchmark::State &state) {
        int i = 0;
        while (state.KeepRunning()) {
            auto mod = turbo::Duration::seconds(i) % turbo::Duration::milliseconds(1);
            benchmark::DoNotOptimize(mod);
            ++i;
        }
    }
...
//
    ...
    TURBO_NOINLINE timespec DoubleToTimespec(double seconds) {
        timespec ts;
        ts.tv_sec = seconds;
    ...
    TURBO_NOINLINE double TimespecToDouble(timespec ts) {
