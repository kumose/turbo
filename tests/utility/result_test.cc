...
#define EXPECT_DEATH_OR_THROW(statement, status) \
  EXPECT_DEATH_IF_SUPPORTED(statement, status.to_string());
    ...
    TEST(StatusOrDeathTest, TestDefaultCtorValue) {
        turbo::Result<int> thing;
        EXPECT_DEATH_OR_THROW(thing.value_or_die(), turbo::unknown_error(""));
        const turbo::Result<int> thing2;
        EXPECT_DEATH_OR_THROW(thing2.value_or_die(), turbo::unknown_error(""));
    }

    TEST(StatusOrDeathTest, TestValueNotOk) {
        turbo::Result<int> thing(turbo::cancelled_error());
        EXPECT_DEATH_OR_THROW(thing.value_or_die(), turbo::cancelled_error());
    }
    ...
    TEST(Result, ValueAccessor) {
        const int kIntValue = 110;
        {
            ...
            EXPECT_EQ(kIntValue, status_or.value_or_die());
            EXPECT_EQ(kIntValue, std::move(status_or).value_or_die());
        }
        {
            turbo::Result<CopyDetector> status_or(kIntValue);
            EXPECT_THAT(status_or,
                        IsOkAndHolds(CopyDetectorHas(kIntValue, false, false)));
            CopyDetector copy_detector = status_or.value_or_die();
            EXPECT_THAT(copy_detector, CopyDetectorHas(kIntValue, false, true));
            copy_detector = std::move(status_or).value_or_die();
    ...
    TEST(Result, BadValueAccess) {
        ...
        EXPECT_DEATH_OR_THROW(status_or.value_or_die(), kError);
    }
...
// turbo::Result<T>
    ...
    TEST(ResultTest, MapFunctionToConstValue) {
        static auto error = turbo::Status(turbo::StatusCode::kInvalidArgument, "some error message");

        const turbo::Result<MoveOnlyDataType> result(MoveOnlyDataType{kIntElement});

        auto const_mapped =
                result.mapping([](const MoveOnlyDataType& m) -> turbo::Result<int> { return *m.data; });
        EXPECT_TRUE(const_mapped.ok());
        EXPECT_EQ(const_mapped.value_or_die(), kIntElement);

        auto const_error =
                result.mapping([](const MoveOnlyDataType& m) -> turbo::Result<int> { return error; });
    ...
    TEST(ResultTest, MapFunctionToRrefValue) {
        ...
        auto result = [] { return turbo::Result<MoveOnlyDataType>(MoveOnlyDataType{kIntElement}); };

        auto move_mapped =
                result().mapping([](MoveOnlyDataType m) -> turbo::Result<int> { return std::move(*m.data); });
        EXPECT_TRUE(move_mapped.ok());
        EXPECT_EQ(move_mapped.value_or_die(), kIntElement);

        auto move_error = result().mapping([](MoveOnlyDataType m) -> turbo::Result<int> { return error; });
    ...
    TEST(ResultTest, MapFunctionToConstError) {
        static auto error = turbo::Status(turbo::StatusCode::kInvalidArgument,"some error message");
        static auto other_error = turbo::Status(turbo::StatusCode::kInvalidArgument,"some other error message");

        const turbo::Result<MoveOnlyDataType> result(error);
        ...
        auto const_error =
                result.mapping([](const MoveOnlyDataType& m) -> turbo::Result<int> { return other_error; });
        EXPECT_FALSE(const_error.ok());
    ...
    TEST(ResultTest, MapFunctionToRrefError) {
        ...
        auto result = [] { return turbo::Result<MoveOnlyDataType>(error); };
        ...
        auto move_error =
                result().mapping([](MoveOnlyDataType m) -> turbo::Result<int> { return other_error; });
        EXPECT_FALSE(move_error.ok());
