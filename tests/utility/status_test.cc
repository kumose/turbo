...
namespace {
    ...
    constexpr ErrorTest kErrorTests[]{
            {turbo::StatusCode::kCancelled,          turbo::Status::cancelled_error,    turbo::is_cancelled},
            {turbo::StatusCode::kUnknown,            turbo::Status::unknown_error,      turbo::is_unknown},
            {turbo::StatusCode::kInvalidArgument,    turbo::Status::invalid_argument_error,
                                                                                turbo::is_invalid_argument},
            {turbo::StatusCode::kDeadlineExceeded,   turbo::Status::deadline_exceeded_error,
                                                                                turbo::is_deadline_exceeded},
            {turbo::StatusCode::kNotFound,           turbo::Status::not_found_error,    turbo::is_not_found},
            {turbo::StatusCode::kAlreadyExists,      turbo::Status::already_exists_error,
                                                                                turbo::is_already_exists},
            {turbo::StatusCode::kPermissionDenied,   turbo::Status::permission_denied_error,
                                                                                turbo::is_permission_denied},
            {turbo::StatusCode::kResourceExhausted,  turbo::Status::resource_exhausted_error,
                                                                                turbo::is_resource_exhausted},
            {turbo::StatusCode::kFailedPrecondition, turbo::Status::failed_precondition_error,
                                                                                turbo::is_failed_precondition},
            {turbo::StatusCode::kAborted,            turbo::Status::aborted_error,      turbo::is_aborted},
            {turbo::StatusCode::kOutOfRange,         turbo::Status::out_of_range_error, turbo::is_out_of_range},
            {turbo::StatusCode::kUnimplemented,      turbo::Status::unimplemented_error,
                                                                                turbo::is_unimplemented},
            {turbo::StatusCode::kInternal,           turbo::Status::internal_error,     turbo::is_internal},
            {turbo::StatusCode::kUnavailable,        turbo::Status::unavailable_error,
                                                                                turbo::is_unavailable},
            {turbo::StatusCode::kDataLoss,           turbo::Status::data_loss_error,    turbo::is_data_loss},
            {turbo::StatusCode::kUnauthenticated,    turbo::Status::unauthenticated_error,
                                                                                turbo::is_unauthenticated},
            {turbo::StatusCode::kIOError,    turbo::Status::io_error,
                    turbo::is_io_error},
    };
    ...
    TEST(Status, TestGetSetPayload) {
        ...
        ok_status.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
        ok_status.set_cord_payload(kUrl2, turbo::Cord(kPayload2));

        EXPECT_FALSE(ok_status.get_cord_payload(kUrl1));
        EXPECT_FALSE(ok_status.get_cord_payload(kUrl2));

        turbo::Status bad_status(turbo::StatusCode::kInternal, "fail");
        bad_status.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
        bad_status.set_cord_payload(kUrl2, turbo::Cord(kPayload2));

        EXPECT_THAT(bad_status.get_cord_payload(kUrl1), Optional(Eq(kPayload1)));
        EXPECT_THAT(bad_status.get_cord_payload(kUrl2), Optional(Eq(kPayload2)));

        EXPECT_FALSE(bad_status.get_cord_payload(kUrl3));

        bad_status.set_cord_payload(kUrl1, turbo::Cord(kPayload3));
        EXPECT_THAT(bad_status.get_cord_payload(kUrl1), Optional(Eq(kPayload3)));

        // Testing dynamically generated type_url
        bad_status.set_cord_payload(turbo::str_cat(kUrl1, ".1"), turbo::Cord(kPayload1));
        EXPECT_THAT(bad_status.get_cord_payload(turbo::str_cat(kUrl1, ".1")),
    ...
    TEST(Status, TestErasePayload) {
        ...
        bad_status.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
        bad_status.set_cord_payload(kUrl2, turbo::Cord(kPayload2));
        bad_status.set_cord_payload(kUrl3, turbo::Cord(kPayload3));

        EXPECT_FALSE(bad_status.erase_payload(kUrl4));

        EXPECT_TRUE(bad_status.get_cord_payload(kUrl2));
        EXPECT_TRUE(bad_status.erase_payload(kUrl2));
        EXPECT_FALSE(bad_status.get_cord_payload(kUrl2));
    ...
    TEST(Status, TestComparePayloads) {
        ...
        bad_status1.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
        bad_status1.set_cord_payload(kUrl2, turbo::Cord(kPayload2));
        bad_status1.set_cord_payload(kUrl3, turbo::Cord(kPayload3));

        turbo::Status bad_status2(turbo::StatusCode::kInternal, "fail");
        bad_status2.set_cord_payload(kUrl2, turbo::Cord(kPayload2));
        bad_status2.set_cord_payload(kUrl3, turbo::Cord(kPayload3));
        bad_status2.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
    ...
    TEST(Status, TestComparePayloadsAfterErase) {
        ...
        payload_status.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
        payload_status.set_cord_payload(kUrl2, turbo::Cord(kPayload2));
    ...
    PayloadsVec AllVisitedPayloads(const turbo::Status &s) {
        ...
        s.for_each_payload([&](std::string_view type_url, const std::shared_ptr<turbo::StatusPayload> &payload) {
            result.push_back(std::make_pair(std::string(type_url), payload->to_string()));
        });
    ...
    TEST(Status, TestForEachPayload) {
        ...
        bad_status.for_each_payload(
                [&count](std::string_view, const std::shared_ptr<turbo::StatusPayload> &) { ++count; });
        ...
        while (true) {
            ...
            scratch.back().set_cord_payload(kUrl1, turbo::Cord(kPayload1));
            scratch.back().set_cord_payload(kUrl2, turbo::Cord(kPayload2));
            scratch.back().set_cord_payload(kUrl3, turbo::Cord(kPayload3));
    ...
    TEST(Status, to_string) {
        ...
        status.set_cord_payload("foo", turbo::Cord("bar"));
        EXPECT_EQ("INTERNAL: fail [foo='bar']", status.to_string());
        status.set_cord_payload("bar", turbo::Cord("\377"));
    ...
    TEST(Status, CopyOnWriteForErasePayload) {
        {
            ...
            base.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
            EXPECT_TRUE(base.get_cord_payload(kUrl1).has_value());
            turbo::Status copy = EraseAndReturn(base);
            EXPECT_TRUE(base.get_cord_payload(kUrl1).has_value());
            EXPECT_FALSE(copy.get_cord_payload(kUrl1).has_value());
        }
        {
            turbo::Status base(turbo::StatusCode::kInvalidArgument, "fail");
            base.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
            ...
            EXPECT_TRUE(base.get_cord_payload(kUrl1).has_value());
            EXPECT_TRUE(copy.get_cord_payload(kUrl1).has_value());

            EXPECT_TRUE(base.erase_payload(kUrl1));

            EXPECT_FALSE(base.get_cord_payload(kUrl1).has_value());
            EXPECT_TRUE(copy.get_cord_payload(kUrl1).has_value());
        }
    }
    ...
    TEST(Status, CopyConstructor) {
        ...
        {
            turbo::Status status(turbo::StatusCode::kInvalidArgument, "message");
            status.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
            turbo::Status copy(status);
            EXPECT_EQ(copy, status);
        }
    }
    ...
    TEST(Status, CopyAssignmentIsNotRef) {
        ...
        status_copy.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
    ...
    TEST(Status, Equality) {
        ...
        one_payload.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
        turbo::Status two_payloads = one_payload;
        two_payloads.set_cord_payload(kUrl2, turbo::Cord(kPayload2));
    ...
    TEST(Status, Swap) {
        ...
        with_payload.set_cord_payload(kUrl1, turbo::Cord(kPayload1));
    ...
    void a1() {
        ...
        for(auto &it : result) {
            std::cout<<std::string_view(static_cast<const char*>(it))<<std::endl;
        }
    }
    void ab(){
        a1();
    ...
    TEST(StatusErrno, with_message) {
        turbo::Status status = turbo::errno_to_status(ENOENT, "Cannot open 'path'");
        EXPECT_EQ(status.code(), turbo::StatusCode::kNotFound);
        EXPECT_EQ(status.message(), "Cannot open 'path': No such file or directory");
        auto s1 = status.with_message("a");

        EXPECT_EQ(s1.code(), turbo::StatusCode::kNotFound);
        EXPECT_EQ(s1.message(), "Cannot open 'path': No such file or directory; a");

        auto s2 = status.with_message("b", 42);

        EXPECT_EQ(s2.code(), turbo::StatusCode::kNotFound);
        EXPECT_EQ(s2.message(), "Cannot open 'path': No such file or directory; b42");
        //ab();
    }

    TEST(signal_from_status_payload, Basics) {
        turbo::Status st;
        st = turbo::OkStatus();
        ASSERT_EQ(signal_from_status_payload(st), 0);
        st = turbo::invalid_argument_error("foo");
        ASSERT_EQ(signal_from_status_payload(st), 0);
        st = turbo::cancelled_error("foo");
        ASSERT_EQ(signal_from_status_payload(st), 0);
        st = status_with_signal_payload(SIGINT, turbo::StatusCode::kInvalidArgument, "foo");
        ASSERT_EQ(signal_from_status_payload(st), SIGINT);
        ASSERT_THAT(st.to_string(),
                    testing::HasSubstr("received signal " + std::to_string(SIGINT)));
        st = turbo::cancelled_with_signal_payload(SIGINT, "bar");
        ASSERT_EQ(signal_from_status_payload(st), SIGINT);
        ASSERT_THAT(st.to_string(),
                    testing::HasSubstr("received signal " + std::to_string(SIGINT)));

        st = turbo::io_error_with_errno_payload(EINVAL, "foo");
        ASSERT_EQ(signal_from_status_payload(st), 0);
    }

    TEST(errno_from_status_payload, Basics) {
        turbo::Status st;
        st = turbo::OkStatus();
        ASSERT_EQ(errno_from_status_payload(st), 0);
        st = turbo::invalid_argument_error("foo");
        ASSERT_EQ(errno_from_status_payload(st), 0);
        st = turbo::io_error("foo");
        ASSERT_EQ(errno_from_status_payload(st), 0);
        st = status_with_errno_payload(EINVAL, turbo::StatusCode::kIOError, "foo");
        ASSERT_EQ(errno_from_status_payload(st), EINVAL);
        st = turbo::io_error_with_errno_payload(EPERM, "foo");
        ASSERT_EQ(errno_from_status_payload(st), EPERM);
        st = turbo::io_error_with_errno_payload(6789, "foo");
        ASSERT_EQ(errno_from_status_payload(st), 6789);

        st = turbo::cancelled_with_signal_payload(SIGINT, "foo");
        ASSERT_EQ(errno_from_status_payload(st), 0);
    }
