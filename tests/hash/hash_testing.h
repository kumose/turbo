...
// operator and the turbo_hash_value implementations.
...
//      suffix of the other.
//   5. turbo_hash_value overloads should not be called by the user. They are only
...
//   int a, b;
//   template <typename H>
//   friend H turbo_hash_value(H state, Bad2 x) {
//     // Uses a and b.
...
//   int a, b;
//   template <typename H>
//   friend H turbo_hash_value(H state, Bad3 x) {
//     // Only uses a.
...
//   int *p, size;
//   template <typename H>
//   friend H turbo_hash_value(H state, Bad4 x) {
...
VerifyTypeImplementsTurboHashCorrectly(const Container& values, Eq equals) {
  ...
  // Now we verify that turbo_hash_value is also correctly implemented.
  ...
  for (const auto& c : classes) {
    ...
    for (const Info& v : c) {
      if (v.expand() != v.expand()) {
        return testing::AssertionFailure()
               << "Hash expansion for " << v.to_string()
               << " is non-deterministic.";
      }
      if (v.expand() != expected) {
        return testing::AssertionFailure()
               << "Values " << c[0].to_string() << " and " << v.to_string()
    ...
    for (const auto& c2 : classes) {
      ...
      switch (SpyHashState::Compare(expected, c2_hash)) {
        case SpyHashState::CompareResult::kEqual:
          return testing::AssertionFailure()
                 << "Values " << c[0].to_string() << " and " << c2[0].to_string()
                 << " evaluate as unequal but have an equal hash expansion.";
        case SpyHashState::CompareResult::kBSuffixA:
          return testing::AssertionFailure()
                 << "Hash expansion of " << c2[0].to_string()
                 << " is a suffix of the hash expansion of " << c[0].to_string()
                 << ".";
        case SpyHashState::CompareResult::kASuffixB:
          return testing::AssertionFailure()
                 << "Hash expansion of " << c[0].to_string()
                 << " is a suffix of the hash expansion of " << c2[0].to_string()
                 << ".";
