...
// ---------------------------------------
//
// When a `KLOG` statement can be trivially proved at compile time to never fire,
...
// `KLOG` statements that ought to be compiled out (e.g. `KLOG_IF(INFO, false)`)
...
//
//   KLOG_IF(INFO, false) << "you're the man now dog";
 ...
 protected:
  ...
  std::unique_ptr<FILE, std::function<void(FILE*)>> OpenTestExecutable() {
    ...
    if (!fp) {
      ...
      turbo::FPrintF(stderr, "Failed to open /proc/self/exe: %s\n", err);
    }
    return fp;
    ...
    if (!fp) {
      const std::string err = turbo::base_internal::StrError(errno);
      turbo::FPrintF(stderr, "Failed to open /pkg/bin/<binary name>: %s\n", err);
    }
    return fp;
    ...
    if (!fp) {
      const std::string err = turbo::base_internal::StrError(errno);
      turbo::FPrintF(stderr, "Failed to open executable at %s: %s\n", path, err);
    }
    return fp;
#elif defined(_WIN32)
    ...
    if (!fp) turbo::FPrintF(stderr, "Failed to open executable\n");
    return fp;
#else
    turbo::FPrintF(stderr,
...
TEST_F(StrippingTest, Fatal) {
  ...
  // We don't care if the KLOG statement is actually executed, we're just
