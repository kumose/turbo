...
TEST(ScopedMockLogTest, CapturesLogsAfterStartCapturingLogs) {
  ...
  // Only this KLOG will be seen by the ScopedMockLog.
...
TEST(ScopedMockLogTest, DoesNotCaptureLogsAfterStopCapturingLogs) {
  ...
  // This KLOG should be seen by the ScopedMockLog.
  KLOG(INFO) << "Expected info";
...
// Tests that no sequence will be imposed on two KLOG message expectations from
// different threads. This test would actually deadlock if replaced to two KLOG
