...
#else
#define TURBO_SYMBOLIZE_TEST_NOINLINE TURBO_NOINLINE
#endif
...
void TURBO_NOINLINE TestWithReturnAddress() {
#if defined(TURBO_HAVE_ATTRIBUTE_NOINLINE)
  void *return_address = __builtin_return_address(0);
  const char *symbol = TrySymbolize(return_address);
  KCHECK_NE(symbol, nullptr) << "TestWithReturnAddress failed";
  KCHECK_STREQ(symbol, "main") << "TestWithReturnAddress failed";
...
void *TURBO_NOINLINE non_inline_func() {
  void *pc = nullptr;
#if defined(__i386__)
  __asm__ __volatile__("call 1f;\n 1: pop %[PC]" : [PC] "=r"(pc));
#elif defined(__x86_64__)
...
void TURBO_NOINLINE TestWithPCInsideNonInlineFunction() {
#if defined(TURBO_HAVE_ATTRIBUTE_NOINLINE) && \
    (defined(__i386__) || defined(__x86_64__))
  void *pc = non_inline_func();
  const char *symbol = TrySymbolize(pc);
  KCHECK_NE(symbol, nullptr) << "TestWithPCInsideNonInlineFunction failed";
  KCHECK_STREQ(symbol, "non_inline_func")
      << "TestWithPCInsideNonInlineFunction failed";
  std::cout << "TestWithPCInsideNonInlineFunction passed" << std::endl;
#endif
}

void TURBO_NOINLINE TestWithPCInsideInlineFunction() {
#if defined(TURBO_HAVE_ATTRIBUTE_ALWAYS_INLINE) && \
    (defined(__i386__) || defined(__x86_64__))
  void *pc = inline_func();  // Must be inlined.
  const char *symbol = TrySymbolize(pc);
  KCHECK_NE(symbol, nullptr) << "TestWithPCInsideInlineFunction failed";
  KCHECK_STREQ(symbol, __FUNCTION__) << "TestWithPCInsideInlineFunction failed";
...
void TURBO_NOINLINE TestArmThumbOverlap() {
#if defined(TURBO_HAVE_ATTRIBUTE_NOINLINE)
  const char *symbol = TrySymbolize((void *)&ArmThumbOverlapArm);
  KCHECK_NE(symbol, nullptr) << "TestArmThumbOverlap failed";
  KCHECK_STREQ("ArmThumbOverlapArm()", symbol) << "TestArmThumbOverlap failed";
