...
#include <turbo/container/pooled_map.h>
...
namespace {
    ...
    TEST_F(FlatMapTest, swap_pooled_allocator) {
        turbo::details::PooledAllocator<int, 128> a1;
        a1.allocate(1);
        const void *p1 = a1._pool._blocks;

        turbo::details::PooledAllocator<int, 128> a2;
        a2.allocate(1);
    ...
    TEST_F(FlatMapTest, copy_flat_map) {
        typedef turbo::FlatMap<std::string, std::string> Map;
        Map uninit_m1;
        ...
        for (size_t i = 0; i < TURBO_ARRAYSIZE(bcs); ++i) {
            Map m5;
    ...
    TEST_F(FlatMapTest, seek_by_string_piece) {
        turbo::FlatMap<std::string, int> m;
    ...
    TEST_F(FlatMapTest, to_lower) {
        for (int c = -128; c < 128; ++c) {
            ASSERT_EQ((char) ::tolower(c), turbo::ascii_tolower(c)) << "c=" << c;
        }
    ...
    TEST_F(FlatMapTest, __builtin_ctzl_perf) {
        ...
        tm1.stop();
        KLOG(INFO) << "__builtin_ctzl takes " << tm1.n_elapsed() / (double) N << "ns s=" << s;
    }

    TEST_F(FlatMapTest, case_ignored_map) {
        turbo::CaseIgnoredFlatMap<int> m1;
    ...
    TEST_F(FlatMapTest, make_sure_all_methods_compile) {
        typedef turbo::FlatMap<int, long> M1;
        M1 m1;
        ASSERT_EQ(0, m1.init(32));
    ...
    TEST_F(FlatMapTest, flat_map_of_string) {
        ...
        turbo::FlatMap<std::string, size_t> m1;
        std::map<std::string, size_t> m2;
        turbo::hash_map<std::string, size_t> m3;
        const size_t N = 10000;
        ASSERT_EQ(0, m1.init(N));
        turbo::TimeCost tm1, tm2, tm3;
        size_t sum = 0;
        keys.reserve(N);
        ...
        tm3.stop();
        KLOG(INFO) << "inserting strings takes " << tm1.n_elapsed() / N
                  << " " << tm2.n_elapsed() / N
                  << " " << tm3.n_elapsed() / N;
        ...
        tm3.stop();
        KLOG(INFO) << "finding strings takes " << tm1.n_elapsed() / N
        ...
        KLOG(INFO) << "finding c_strings takes " << tm1.n_elapsed() / N
                  << " " << tm2.n_elapsed() / N << " " << tm3.n_elapsed() / N << " sum=" << sum;
    ...
    TEST_F(FlatMapTest, fast_iterator) {
        typedef turbo::FlatMap<uint64_t, uint64_t> M1;
        typedef turbo::SparseFlatMap<uint64_t, uint64_t> M2;
        ...
        KLOG(INFO) << "m1.insert=" << tm1.n_elapsed() / (double) N
        ...
        tm2.stop();
        KLOG(INFO) << "m1.iterate=" << tm1.n_elapsed() / (double) N
    ...
    static void list_flat_map(std::vector<Key> *keys,
                              const turbo::FlatMap<Key, Value> &map,
                              size_t max_one_pass,
                              OnPause &on_pause) {
        keys->clear();
        typedef turbo::FlatMap<Key, Value> Map;
        size_t n = 0;
    ...
    typedef turbo::FlatMap<uint64_t, uint64_t> PositionHintMap;
    ...
    struct RemoveInsertVisitedOnPause {
        ...
        turbo::FlatSet<uint64_t> removed_keys;
        turbo::FlatSet<uint64_t> inserted_keys;
    ...
    TEST_F(FlatMapTest, erase_hinted_during_iteration) {
        ...
        KLOG_IF(INFO, keys_out.size() != old_keys_out_size)
    ...
    struct PointerHasher {
        size_t operator()(const T *p) const { return fmix64(reinterpret_cast<uint64_t>(p)); }
    };

    template<typename T>
    struct PointerHasher2 {
        size_t operator()(const T *p) const { return reinterpret_cast<uint64_t>(p); }
    };

    TEST_F(FlatMapTest, perf_cmp_with_map_storing_pointers) {
        const size_t REP = 4;
        int *ptr[2048];
        for (size_t i = 0; i < TURBO_ARRAYSIZE(ptr); ++i) {
            ptr[i] = new int;
        }

        std::set<int *> m1;
        turbo::FlatSet<int *, PointerHasher<int> > m2;
        turbo::hash_set<int *, PointerHasher<int> > m3;

        std::vector<int *> r;
        int sum;
        turbo::TimeCost tm;

        r.reserve(TURBO_ARRAYSIZE(ptr) * REP);
        ASSERT_EQ(0, m2.init(TURBO_ARRAYSIZE(ptr)));
        ...
        tm.stop();
        KLOG(INFO) << "FlatMap takes " << tm.n_elapsed() / r.size();
        ...
        tm.stop();
        KLOG(INFO) << "std::set takes " << tm.n_elapsed() / r.size();
        ...
        tm.stop();
        KLOG(INFO) << "std::set takes " << tm.n_elapsed() / r.size() << " sum=" << sum;
    ...
    struct Value {
        ...
        friend std::ostream &operator<<(std::ostream &os, const Value &v) { return os << v.x_; }
    ...
    struct KeyEqualTo {
        size_t operator()(const Key &k1, const Key &k2) const { return k1.x_ == k2.x_; }
    };

    TEST_F(FlatMapTest, key_value_are_not_constructed_before_first_insertion) {
        turbo::FlatMap<Key, Value, KeyHasher, KeyEqualTo> m;
    ...
    TEST_F(FlatMapTest, manipulate_uninitialized_map) {
        turbo::FlatMap<int, int> m;
        ASSERT_FALSE(m.initialized());
        for (turbo::FlatMap<int, int>::iterator it = m.begin(); it != m.end(); ++it) {
            KLOG(INFO) << "nothing";
        }
    ...
    TEST_F(FlatMapTest, perf_small_string_map) {
        ...
        for (int i = 0; i < 10; ++i) {
            tm3.reset();
            turbo::PooledMap<std::string, std::string> m3;
            ...
            tm4.reset();
            turbo::CaseIgnoredFlatMap<std::string> m4;
            m4.init(16);
            ...
            tm1.reset();
            turbo::FlatMap<std::string, std::string> m1;
            m1.init(16);
            ...
            KLOG(INFO) << "flatmap=" << tm1.n_elapsed()
    ...
    TEST_F(FlatMapTest, sanity) {
        typedef turbo::FlatMap<uint64_t, long> Map;
        Map m;
    ...
    TEST_F(FlatMapTest, random_insert_erase) {
        ...
        {
            turbo::hash_map<uint64_t, Value> ref[2];
            typedef turbo::FlatMap<uint64_t, Value> Map;
            Map ht[2];
            ht[0].init(40);
            ht[1] = ht[0];
            ...
            for (int j = 0; j < 30; ++j) {
                ...
                for (int i = 0; i < 2; ++i) {
                    for (Map::iterator it = ht[i].begin(); it != ht[i].end(); ++it) {
                        turbo::hash_map<uint64_t, Value>::iterator it2 = ref[i].find(it->first);
                        ASSERT_TRUE (it2 != ref[i].end());
                        ASSERT_EQ (it2->second, it->second);
                    }

                    for (turbo::hash_map<uint64_t, Value>::iterator it = ref[i].begin();
                         it != ref[i].end(); ++it) {
        ...
        KLOG(INFO) << "n_con:" << n_con << std::endl
    ...
    void perf_insert_erase(bool random, const T &value) {
        ...
        const size_t NPASS = TURBO_ARRAYSIZE(nkeys);

        std::vector<uint64_t> keys;
        turbo::FlatMap<uint64_t, T> id_map;
        turbo::MultiFlatMap<uint64_t, T> multi_id_map;
        std::map<uint64_t, T> std_map;
        turbo::PooledMap<uint64_t, T> pooled_map;
        std::unordered_map<uint64_t, T> std_unordered_map;
        std::unordered_multimap<uint64_t, T> std_unordered_multimap;
        turbo::hash_map<uint64_t, T> hash_map;
        turbo::TimeCost id_tm, multi_id_tm, std_tm, pooled_tm,
        ...
        KLOG(INFO) << "[ value = " << sizeof(T) << " bytes ]";
        for (size_t pass = 0; pass < NPASS; ++pass) {
            ...
            KLOG(INFO) << (random ? "Randomly" : "Sequentially")
                      << " inserting " << keys.size()
                      << " into FlatMap/MultiFlatMap/std::map/turbo::PooledMap/"
                         "std::unordered_map/std::unordered_multimap/turbo::hash_map takes "
            ...
            KLOG(INFO) << (random ? "Randomly" : "Sequentially")
                      << " erasing " << keys.size()
                      << " from FlatMap/MultiFlatMap/std::map/turbo::PooledMap/"
                         "std::unordered_map/std::unordered_multimap/turbo::hash_map takes "
                      << id_tm.n_elapsed() / keys.size()
                      << "/" << multi_id_tm.n_elapsed() / keys.size()
                      << "/" << std_tm.n_elapsed() / keys.size()
    ...
    TEST_F(FlatMapTest, copy) {
        turbo::FlatMap<int, int> m1;
        turbo::FlatMap<int, int> m2;
        ASSERT_EQ(0, m1.init(32));
        m1[1] = 1;
        m1[2] = 2;
        m2 = m1;
    ...
    TEST_F(FlatMapTest, multi) {
        ...
        g_foo_copy_ctor = 0;
        g_foo_assign = 0;
        turbo::MultiFlatMap<int, Foo> map;
        ...
        turbo::DefaultHasher<int> hasher;
        ASSERT_EQ(turbo::flatmap_mod(hasher(1), bucket_count),
                  turbo::flatmap_mod(hasher(same_bucket_key), bucket_count));
        ...
        ASSERT_EQ(turbo::flatmap_round(bucket_count + 1), map.bucket_count());
        ...
        // Zeroize POD values.
        turbo::MultiFlatMap<int, Bar> map2;
        ASSERT_EQ(0, map2.init(32));
        Bar &g = map2[1];
        ASSERT_EQ(0, g.x);
        g.x = 123;
