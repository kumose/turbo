...
namespace turbo {
    ...
    TEST(Uri, ParsePath) {
        ...
        auto check_case = [&](std::string uri_string, std::string scheme, bool has_host,
            ...
            ASSERT_TRUE(uri.parse(uri_string).ok());
...
// With escaped path characters
    ...
    TEST(Uri, ParseQuery) {
        ...
        auto check_case = [&](std::string uri_string, std::string query_string,
            ...
            ASSERT_EQ(result.value_unsafe(), items);
        };
...
// With escaped values
    ...
    TEST(Uri, ParseHostPort) {
        ...
        ASSERT_TRUE(uri.parse("http://localhost:80").ok());
        ...
        ASSERT_TRUE(uri.parse("http://1.2.3.4").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ...
        ASSERT_TRUE(uri.parse("http://1.2.3.4:80").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "1.2.3.4");
        ASSERT_EQ(uri.port_text(), "80");
        ASSERT_EQ(uri.port(), 80);
        ...
        ASSERT_TRUE(uri.parse("http://[::1]").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ...
        ASSERT_TRUE(uri.parse("http://[::1]:80").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "::1");
        ASSERT_EQ(uri.port_text(), "80");
        ASSERT_EQ(uri.port(), 80);
    ...
    TEST(Uri, ParseUserPass) {
        ...
        ASSERT_TRUE(uri.parse("http://someuser@localhost:80").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "localhost");
        ...
        ASSERT_TRUE(uri.parse("http://someuser:somepass@localhost:80").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "localhost");
        ASSERT_EQ(uri.username(), "someuser");
        ASSERT_EQ(uri.password(), "somepass");

        ASSERT_TRUE(uri.parse("http://someuser:somepass@localhost").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "localhost");
        ASSERT_EQ(uri.username(), "someuser");
        ASSERT_EQ(uri.password(), "somepass");
        ...
        // With %-encoding
        ASSERT_TRUE(uri.parse("http://some%20user%2Fname:somepass@localhost").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "localhost");
        ASSERT_EQ(uri.username(), "some user/name");
        ASSERT_EQ(uri.password(), "somepass");

        ASSERT_TRUE(uri.parse("http://some%20user%2Fname:some%20pass%2Fword@localhost").ok());
        ASSERT_EQ(uri.scheme(), "http");
        ASSERT_EQ(uri.host(), "localhost");
        ASSERT_EQ(uri.username(), "some user/name");
    ...
    TEST(Uri, FileScheme) {
        ...
        ASSERT_FALSE(uri.parse("file:").ok());
        ASSERT_FALSE(uri.parse("file:foo/bar").ok());
...
// (authority with special chars, not 100% sure this is the right behavior)
    ...
    TEST(Uri, ParseError) {
        ...
        ASSERT_FALSE(uri.parse("http://a:b:c:d").ok());
        ASSERT_FALSE(uri.parse("http://localhost:z").ok());
        ASSERT_FALSE(uri.parse("http://localhost:-1").ok());
        ASSERT_FALSE(uri.parse("http://localhost:99999").ok());

// Scheme-less URIs (forbidden by RFC 3986, and ambiguous to parse)
        ASSERT_FALSE(uri.parse("localhost").ok());
        ASSERT_FALSE(uri.parse("/foo/bar").ok());
        ASSERT_FALSE(uri.parse("foo/bar").ok());
        ASSERT_FALSE(uri.parse("").ok());
    }

    TEST(uri_from_absolute_path, Basics) {
#ifdef _WIN32
        ASSERT_TRUE(, &ret));
        ASSERT_EQ("file:///C:/foo/bar", uri_from_absolute_path("C:\\foo\\bar").value_or_die);
...
#else
        ASSERT_EQ("file:///", uri_from_absolute_path("/").value_or_die());
        ASSERT_EQ("file:///tmp/foo/bar", uri_from_absolute_path("/tmp/foo/bar").value_or_die());
        ASSERT_EQ("file:///some%20path/100%25%20%C3%A9l%C3%A9phant", uri_from_absolute_path("/some path/100% éléphant").value_or_die());
#endif
    }
