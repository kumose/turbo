...
#define MAKE_INT128(HI, LO) turbo::make_int128(static_cast<int64_t>(HI), LO)
...
TYPED_TEST(Uint128IntegerTraitsTest, ConstructAssignTest) {
  static_assert(std::is_constructible<turbo::uint128, TypeParam>::value,
                "turbo::uint128 must be constructible from TypeParam");
  static_assert(std::is_assignable<turbo::uint128&, TypeParam>::value,
                "turbo::uint128 must be assignable from TypeParam");
  static_assert(!std::is_assignable<TypeParam&, turbo::uint128>::value,
...
TEST(Uint128, AllTests) {
  ...
  EXPECT_EQ(turbo::uint128_high64(biggest), std::numeric_limits<uint64_t>::max());
  EXPECT_EQ(turbo::uint128_low64(biggest), std::numeric_limits<uint64_t>::max());
  EXPECT_EQ(zero + one, one);
  ...
  EXPECT_EQ(turbo::uint128_high64((turbo::uint128(1) << 64) - 1), 0);
  EXPECT_EQ(turbo::uint128_low64((turbo::uint128(1) << 64) - 1),
  ...
  EXPECT_EQ(big, +big);
  EXPECT_EQ(two, +two);
  EXPECT_EQ(turbo::uint128_max(), +turbo::uint128_max());
  EXPECT_EQ(zero, +zero);

  EXPECT_EQ(big, -(-big));
  EXPECT_EQ(two, -((-one) - 1));
  EXPECT_EQ(turbo::uint128_max(), -one);
  EXPECT_EQ(zero, -zero);
}
...
TEST(Uint128, ConversionTests) {
  EXPECT_TRUE(turbo::make_uint128(1, 0));
...
TEST(Uint128, Multiply) {
  ...
  c = a * b;
  EXPECT_EQ(turbo::make_uint128(0x530EDA741C71D4C3, 0xBF25975319080000), c);
  EXPECT_EQ(0, c - b * a);
  ...
  c = a * b;
  EXPECT_EQ(turbo::make_uint128(0x97a87f4f261ba3f2, 0x342d0bbf48948200), c);
  EXPECT_EQ(0, c - b * a);
  EXPECT_EQ(a*a - b*b, (a+b) * (a-b));
}
...
TEST(Uint128, ConstexprTest) {
  ...
  EXPECT_EQ(minus_two, turbo::make_uint128(-1, -2));
}
...
TEST(Uint128, NumericLimitsTest) {
  ...
  EXPECT_EQ(turbo::uint128_max(), std::numeric_limits<turbo::uint128>::max());
}
...
TYPED_TEST(Int128IntegerTraitsTest, ConstructAssignTest) {
  static_assert(std::is_constructible<turbo::int128, TypeParam>::value,
                "turbo::int128 must be constructible from TypeParam");
  static_assert(std::is_assignable<turbo::int128&, TypeParam>::value,
                "turbo::int128 must be assignable from TypeParam");
  static_assert(!std::is_assignable<TypeParam&, turbo::int128>::value,
...
TEST(Int128, BoolConversionTest) {
  ...
  for (int i = 0; i < 64; ++i) {
    EXPECT_TRUE(turbo::make_int128(0, uint64_t{1} << i));
  }
  for (int i = 0; i < 63; ++i) {
    EXPECT_TRUE(turbo::make_int128(int64_t{1} << i, 0));
  }
  EXPECT_TRUE(turbo::int128_min());
...
TYPED_TEST(Int128FloatConversionTest, ConstructAndCastTest) {
  ...
  EXPECT_DOUBLE_EQ(std::ldexp(static_cast<TypeParam>(1), 127),
                   static_cast<TypeParam>(turbo::int128_max()));
  EXPECT_DOUBLE_EQ(-std::ldexp(static_cast<TypeParam>(1), 127),
                   static_cast<TypeParam>(turbo::int128_min()));
}

TEST(Int128, FactoryTest) {
  EXPECT_EQ(turbo::int128(-1), turbo::make_int128(-1, -1));
  EXPECT_EQ(turbo::int128(-31), turbo::make_int128(-1, -31));
  EXPECT_EQ(turbo::int128(std::numeric_limits<int64_t>::min()),
            turbo::make_int128(-1, std::numeric_limits<int64_t>::min()));
  EXPECT_EQ(turbo::int128(0), turbo::make_int128(0, 0));
  EXPECT_EQ(turbo::int128(1), turbo::make_int128(0, 1));
...
TEST(Int128, HighLowTest) {
  ...
  for (const HighLowPair& pair : values) {
    ...
    EXPECT_EQ(pair.low, turbo::int128_low64(value));
    EXPECT_EQ(pair.high, turbo::int128_high64(value));
  }
}

TEST(Int128, LimitsTest) {
  EXPECT_EQ(turbo::make_int128(0x7fffffffffffffff, 0xffffffffffffffff),
            turbo::int128_max());
  EXPECT_EQ(turbo::int128_max(), ~turbo::int128_min());
}
...
TEST(Int128, ConstexprTest) {
  ...
  EXPECT_EQ(minus_two, turbo::make_int128(-1, -2));
  EXPECT_GT(max, one);
...
TEST(Int128, UnaryPlusTest) {
  ...
  for (int64_t value : values64) {
    ...
    EXPECT_EQ(turbo::int128(-value), +turbo::int128(-value));
    EXPECT_EQ(turbo::make_int128(value, 0), +turbo::make_int128(value, 0));
    EXPECT_EQ(turbo::make_int128(-value, 0), +turbo::make_int128(-value, 0));
  }
}
...
TEST(Int128, LogicalNotTest) {
  ...
  for (int i = 0; i < 64; ++i) {
    EXPECT_FALSE(!turbo::make_int128(0, uint64_t{1} << i));
  }
  for (int i = 0; i < 63; ++i) {
    EXPECT_FALSE(!turbo::make_int128(int64_t{1} << i, 0));
  }
}
...
TEST(Int128, AdditionSubtractionTest) {
  ...
  EXPECT_EQ(turbo::make_int128(31, 0),
...
TEST(Int128, MultiplicationTest) {
  ...
  for (const std::pair<int64_t, int64_t>& pair : small_values) {
    ...
    EXPECT_EQ(turbo::make_int128(pair.first * pair.second, 0),
  ...
  EXPECT_EQ(turbo::make_int128(0xcd0efd3442219bb, 0xde47c05bcd9df6e1),
            turbo::make_int128(0x7c6448, 0x3bc4285c47a9d253) * 0x1a6037537b);
  EXPECT_EQ(-turbo::make_int128(0x1f8f149850b1e5e6, 0x1e50d6b52d272c3e),
            -turbo::make_int128(0x23, 0x2e68a513ca1b8859) * 0xe5a434cd14866e);
  EXPECT_EQ(-turbo::make_int128(0x55cae732029d1fce, 0xca6474b6423263e4),
            0xa9b98a8ddf66bc * -turbo::make_int128(0x81, 0x672e58231e2469d7));
  EXPECT_EQ(turbo::make_int128(0x19c8b7620b507dc4, 0xfec042b71a5f29a4),
...
TEST(Int128, DivisionAndModuloTest) {
  ...
  // Min and max values
  EXPECT_EQ(0, turbo::int128_max() / turbo::int128_min());
  EXPECT_EQ(turbo::int128_max(), turbo::int128_max() % turbo::int128_min());
  EXPECT_EQ(-1, turbo::int128_min() / turbo::int128_max());
  EXPECT_EQ(-1, turbo::int128_min() % turbo::int128_max());
  ...
  DivisionModCase manual_cases[] = {
      {turbo::make_int128(0x6ada48d489007966, 0x3c9c5c98150d5d69),
       turbo::make_int128(0x8bc308fb, 0x8cb9cc9a3b803344), 0xc3b87e08,
       turbo::make_int128(0x1b7db5e1, 0xd9eca34b7af04b49)},
      {turbo::make_int128(0xd6946511b5b, 0x4886c5c96546bf5f),
       -turbo::make_int128(0x263b, 0xfd516279efcfe2dc), -0x59cbabf0,
       turbo::make_int128(0x622, 0xf462909155651d1f)},
      {-turbo::make_int128(0x33db734f9e8d1399, 0x8447ac92482bca4d), 0x37495078240,
       -turbo::make_int128(0xf01f1, 0xbc0368bf9a77eae8), -0x21a508f404d},
      {-turbo::make_int128(0x13f837b409a07e7d, 0x7fc8e248a7d73560), -0x1b9f,
       turbo::make_int128(0xb9157556d724, 0xb14f635714d7563e), -0x1ade},
  };
...
TEST(Int128, BitwiseLogicTest) {
  ...
  for (const std::pair<int64_t, int64_t>& pair : pairs64) {
    ...
    EXPECT_EQ(turbo::make_int128(~pair.first, ~pair.second),
...
TEST(Int128, BitwiseShiftTest) {
  ...
  for (int i = 0; i < 63; ++i) {
    for (int j = 0; j < 64; ++j) {
      ...
      SCOPED_TRACE(::testing::Message() << "i = " << i << "; j = " << j);
      EXPECT_EQ(turbo::make_int128(uint64_t{1} << i, 0),
...
TEST(Int128, NumericLimitsTest) {
  ...
  EXPECT_EQ(turbo::int128_min(), std::numeric_limits<turbo::int128>::min());
  EXPECT_EQ(turbo::int128_min(), std::numeric_limits<turbo::int128>::lowest());
  EXPECT_EQ(turbo::int128_max(), std::numeric_limits<turbo::int128>::max());
}
