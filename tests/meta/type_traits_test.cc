...
TEST(NegationTest, BasicBooleanLogic) {
  EXPECT_FALSE(std::negation<std::true_type>::value);
  EXPECT_FALSE(std::negation<MyTrueType>::value);
  EXPECT_TRUE(std::negation<std::false_type>::value);
  EXPECT_TRUE(std::negation<MyFalseType>::value);
}
...
TEST(TypeTraitsTest, TestIsFunction) {
  ...
  EXPECT_TRUE(std::is_function<void()>::value);
  EXPECT_TRUE(std::is_function<void()&>::value);
  EXPECT_TRUE(std::is_function<void() const>::value);
  EXPECT_TRUE(std::is_function<void() noexcept>::value);
  EXPECT_TRUE(std::is_function<void(...) noexcept>::value);

  EXPECT_FALSE(std::is_function<void (*)()>::value);
  EXPECT_FALSE(std::is_function<void (&)()>::value);
  EXPECT_FALSE(std::is_function<int>::value);
  EXPECT_FALSE(std::is_function<Callable>::value);
}
...
TEST(TriviallyRelocatable, UserProvidedCopyAssignment) {
  ...
  static_assert(!turbo::is_trivially_relocatable<S>::value, "errr");
}
...
TEST(TriviallyRelocatable, UserProvidedMoveAssignment) {
  ...
  static_assert(!turbo::is_trivially_relocatable<S>::value, "1");
}
