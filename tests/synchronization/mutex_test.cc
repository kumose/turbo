...
static void TestTime(TestContext *cxt, int c, bool use_cv) {
  ...
  if (c == 0) {
    ...
    if (use_cv) {
      ...
      KCHECK(!cxt->mu.AwaitWithTimeout(false_cond, turbo::Duration::seconds(1)))
          << "TestTime failed";
    }
    turbo::Duration elapsed = turbo::Time::current_time() - start;
    KCHECK(turbo::Duration::seconds(0.9) <= elapsed && elapsed <= turbo::Duration::seconds(2.0))
        << "TestTime failed";
    ...
    if (use_cv) {
      ...
      KCHECK(!cxt->mu.AwaitWithTimeout(false_cond, turbo::Duration::seconds(4)))
          << "TestTime failed";
    }
    elapsed = turbo::Time::current_time() - start;
    KCHECK(turbo::Duration::seconds(3.9) <= elapsed && elapsed <= turbo::Duration::seconds(6.0))
        << "TestTime failed";
    ...
    if (use_cv) {
      ...
      KCHECK(!cxt->mu.AwaitWithTimeout(false_cond, turbo::Duration::milliseconds(500)))
          << "TestTime failed";
    }
    const turbo::Duration elapsed = turbo::Time::current_time() - start;
    KCHECK(turbo::Duration::seconds(0.4) <= elapsed && elapsed <= turbo::Duration::seconds(0.9))
        << "TestTime failed";
    cxt->g0++;
  } else if (c == 2) {
    turbo::MutexLock l(&cxt->mu);
    if (use_cv) {
      ...
      KCHECK(cxt->mu.AwaitWithTimeout(g0ge2, turbo::Duration::seconds(100)))
          << "TestTime failed";
    }
    cxt->g0++;
  } else {
