...
namespace {
    ...
    static_assert(!turbo::type_traits_internal::IsOwner<std::string_view>::value &&
                  turbo::type_traits_internal::IsView<std::string_view>::value,
                  "string_view is a view, not an owner");

    static_assert(turbo::type_traits_internal::IsLifetimeBoundAssignment<
                          std::string_view, std::string>::value,
...
#else
        TURBO_EXPECT_DEATH_IF_SUPPORTED(a.copy(buf, 1, 27), "std::string_view::copy");
#endif
    }
...
// Continued from STL2
...
#else
        TURBO_EXPECT_DEATH_IF_SUPPORTED((void)abc.at(3), "std::string_view::at");
#endif
    }
    ...
    TEST(StringViewTest, starts_with) {
      ...
      const std::string_view b("123\0abc", 7);
      const std::string_view e;
      ...
      constexpr std::string_view kBar("bar");
