...
namespace {
    ...
    TEST(PopenTest, posix_popen) {
        ...
        ASSERT_EQ(0, rc) << km_error(errno);
        ASSERT_EQ("Hello World\n", oss.str());

        oss.str("");
        rc = turbo::read_command_output_through_popen(oss, "exit 1");
        EXPECT_EQ(1, rc) << km_error(errno);
        ...
        /*
        oss.str("");
         ASSERT_EQ(0, turbo::read_command_output_through_popen(oss, "for i in `seq 1 100000`; do echo -n '=' ; done"));
